...
The Buckingham-$\pi$ theorem reduces this to 15 dimensionless variables $\mathbf{\Pi_1}$ without approximation (see A).
...
A shows the equation for an arbitrary number of poles and zeros.
...
A shows a fit to describe radiation impedance $\hat{Z}_h$ as a function of $kh$ using only two parameters.
...
\Cref{fig:cem-cost-sweep} shows the normalized grid cost, CO2 emissions, and capacity from the CEM for a WEC with $\zeta=0.05, \omega_n=0.4$, and $B_{u}=10^5\omega e^{-\omega^2}$ (a Rayleigh distribution: an alternative model to the one in A) in the Northeast.
