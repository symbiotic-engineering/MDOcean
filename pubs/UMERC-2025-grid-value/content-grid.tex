...
\figureautorefname~\ref{umerc:fig:n2} depicts the optimization structure in an xDSM diagram \cite{lambe_extensions_2012}, with design variables on the top, objectives and constraints in the rightmost two columns, and simulation modules along the diagonal.
...
Grey lines in \figureautorefname~\ref{umerc:fig:n2} show module connections.
...
\appendixname~A shows a fit to describe radiation impedance $\hat{Z}_h$ as a function of $kh$ using only two parameters.
...
These are scope 3 eco-cost coefficients \cite{vogtlander_lca-based_2010} that express diverse lifetime environmental impacts (i.e. water, emissions, pollution) of various materials and processes in monetary units (see \tableautorefname~\ref{tab:lca-weights}).
