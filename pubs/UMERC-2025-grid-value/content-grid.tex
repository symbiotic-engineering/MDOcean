...
The Buckingham-$\pi$ theorem reduces this to 15 dimensionless variables $\gls{sym-Pi-1}$ without approximation (see \cref{sec:appendix-grid}).
To further reduce, we leverage rigid body dynamics, which dictates the following:
\begin{equation}
    \label{eq:CW-fraction}
    \frac{CW}{CW_{max}} = \frac{4 \mathcal{D}\eta_{pto}}{G} \frac{\omega^2}{g k}  \frac{\Re(\hat{Z}_h(\gls{sym-Pi-1}))~ \Re(\hat{Z}_u(\gls{sym-Pi-1}))}{\left|\hat{Z}_h(\gls{sym-Pi-1})+\hat{Z}_u(\gls{sym-Pi-1})+\hat{Z}_d(\gls{sym-Pi-1})\right|^2}
\end{equation}
...
The ROM seeks to approximate the three functions $\hat{Z}(\gls{sym-Pi-1})$ as functions of a reduced set of values $\gls{sym-Pi-2}$.
Before the design optimization, the CEM is run for all combinations of $\gls{sym-Pi-2}$.
During design optimization, we perform a least-squares fit over an error model to find the reduced groups $\gls{sym-Pi-2}$ at each iteration, and then use a surrogate model to find CEM outputs as a function of $\gls{sym-Pi-2}$. 
...
Two approaches are possible in selecting $\gls{sym-Pi-2}$ and the corresponding error model: a pole-zero description of the dynamics to obtain a low-order transfer function, or a hydrodynamics-informed curve fit.
A pole-zero description, $\gls{sym-Pi-2-pz}$, is easy to implement via standard system identification tools, but may require a high order to adequately capture the dynamics. 
...
Alternatively, a custom hydrodynamics curve fit, $\gls{sym-Pi-2-hyd}$, offers more accuracy at lower order using deep mechanistic understanding of the physics.
...
This study evaluates a second-order pole-zero model $\gls{sym-Pi-2-pz-zeta-omega-n}$ to fit the closed loop system compliance, $\hat{X}/\hat{F}=1/(i\omega(\hat{Z}_h+\hat{Z}_u+\hat{Z}_d))$,  but finds a poor fit.
...
Future work should better explore hydrodynamic curve fits $\gls{sym-Pi-2-hyd}$.
...
The slope has a useful sensitivity interpretation: for every $30\%$ reduction in WEC cost, system cost reduces by $10\%$.
