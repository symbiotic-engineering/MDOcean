...
\begin{table}[htbp]
    ...
    Vertical eigen-function $\gls{sym-Z}(\gls{sym-z})$
        ...
        \begin{cases}
            1/\sqrt{2}
                & \gls{sym-n}=0\\[1em]
            \cos\!\left(\gls{sym-lambda}_{\gls{sym-n}}^{i1}
                (\gls{sym-z}+\gls{sym-h})
            \right)
                & \gls{sym-n}\ge1
        \end{cases}$
        ...
        \begin{cases}
            1/\sqrt{2}
                & \gls{sym-m}=0\\[1em]
            \cos\!\left(\gls{sym-lambda}_{\gls{sym-m}}^{i2}
                (\gls{sym-z}+\gls{sym-h})
            \right)
                & \gls{sym-m}\ge1
        \end{cases}$
...
\newcommand{\RintTwoJOne}{
    ...
    \frac
    {
        \gls{sym-a}_{in}\,\gls{sym-mathrm-K-1} (\gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-in})
        -\gls{sym-a}_{out}\,{\mathrm{\gls{sym-K}}}_1(\gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-out})
    }
    {
        \gls{sym-vec-lambda}_{\gls{sym-m}} \,{\mathrm{\gls{sym-K}}}_0 (\gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-2})
    }
    $
}
...
\newcommand{\ZnZeroMOne}{
    $\displaystyle
     \frac
                {\sqrt{2}\sin\left(\gls{sym-vec-lambda}_{\gls{sym-m}} (\gls{sym-h} - \gls{sym-d-1})
			\right)}
...
\frac{
            ...
            {
                    1 - \left(\frac{\gls{sym-vec-lambda}_{\gls{sym-n}}^T}{  \gls{sym-vec-lambda}_{\gls{sym-m}}}
                        \right)^2
            }
\frac{
            \sin\left(\gls{sym-vec-lambda}_{\gls{sym-m}} (\gls{sym-h}-\gls{sym-d-1})
         	\right)
            }
            {
            \gls{sym-vec-lambda}_{\gls{sym-m}}
...
%     $\displaystyle
%     \frac
%         {\sin\left(% 		(\gls{sym-vec-lambda}_{\gls{sym-n}}^T + \gls{sym-vec-lambda}_{\gls{sym-m}}) (\gls{sym-h}-\gls{sym-d-1})
% 	\right)}
...
Once orthogonality is applied, the matching equations create a linear system $\gls{sym-A}\gls{sym-vec-x}=\gls{sym-vec-b}$ where $\gls{sym-A}$ is a complex sparse $(\gls{sym-N}+2\gls{sym-M}+\gls{sym-K})$x$(\gls{sym-N}+2\gls{sym-M}+\gls{sym-K})$ square matrix corresponding to the homogeneous case, $\gls{sym-vec-x}=[\gls{sym-vec-C-1n-i1}, \gls{sym-vec-C-1m-i2}, \gls{sym-vec-C-2m-i2}, \vec{\gls{sym-B}_{\gls{sym-k-index}}^{e}}]$ is the complex eigencoefficient vector, and $\gls{sym-vec-b}$ is the real boundary condition vector corresponding to the particular case.
...
$\\ \hline
         K& $\displaystyle
\int_{-\gls{sym-h}}^{-\gls{sym-d-2}} \frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-p}^{i2} \vec{\gls{sym-Z}}_{\gls{sym-k-index}}^{\textit{e}~\gls{sym-T}} dz
$\\ \hline
    \end{tabular}
\end{table}
...
\begin{tabular}{|l|c|c|c|c|} \hline
          Body&N&  M&  M& K\\ \hline
          Float&$0_{1\gls{sym-N}}$&  $\gls{sym-vec-Z-m}(\gls{sym-z}=-d) \odot\gls{sym-boldsymbol-mathcal-R-1m}$&  $\gls{sym-vec-Z-m}(\gls{sym-z}=-d) \odot\gls{sym-boldsymbol-mathcal-R-2m}$& $0_{1\gls{sym-K}}$\\ \hline
          Spar& $\gls{sym-vec-Z-n}(\gls{sym-z}=-d) \odot\gls{sym-boldsymbol-mathcal-R-1n}$& $0_{1\gls{sym-M}}$& $0_{1\gls{sym-M}}$&$0_{1\gls{sym-K}}$\\\hline
    \end{tabular}
    \end{table}
...
\begin{equation}\label{eq:gamma-K}
   ...
   \angle \gls{sym-gamma} = -\frac{\gls{sym-pi}}{2} + \angle\frac{ \gls{sym-B-k-0-e}}{\textrm{__PAPERLINT_GLS_3__}_0^{1} (\gls{sym-m-0} \gls{sym-a-2})},\quad
...
\end{equation}
where $\gls{sym-g}$ is the acceleration due to gravity, $\gls{sym-rho-w}$ is the density of water, $\gls{sym-m-0}$ is the wavenumber, $\gls{sym-V-g}$ is the finite depth group velocity, $\textrm{\textrm{__PAPERLINT_GLS_0__}}_0^1$ is the zeroth-order Hankel function of the first kind, and $\gls{sym-A-w}$ is the waterplane area \citep{newman}.
...
\begin{equation}
    \lim_{m_0h\rightarrow\infty} Z_0^\textit{e} (z)= \frac{\cosh^2 (m_0h)}{\sqrt{2m_0h}}\exp\left(1+\frac{z}{h}\right)
\end{equation}
...
\begin{equation}
\begin{aligned}
    \lim_{\gls{sym-m-0}\gls{sym-h}\rightarrow\infty}\gls{sym-boldsymbol-mathcal-Z-m-k-0} &
    %= \int_{-h}^{-d_2} \vec{Z}_m^{i2 ~T}\vec{Z}_0^{\textit{e}} dz
    = \gls{sym-h}~\frac{\cosh^2 (\gls{sym-m-0}\gls{sym-h})}{\sqrt{2\gls{sym-m-0}\gls{sym-h}}}\cdot \frac{-1+(-1)^\gls{sym-m}\exp(1-\frac{\gls{sym-d-2}}{\gls{sym-h}})}{\gls{sym-f-m}} \\
    \text{where}~~\gls{sym-f-m}&= \begin{cases}
        1, & \gls{sym-m}=0 \\
...
Then $3\gls{sym-N}+8\gls{sym-M}+2\gls{sym-K}-11$ Bessel functions must be evaluated for the radial terms of the A-matrix.
 (Note that this would reduce to $2\gls{sym-N}+8\gls{sym-M}+2\gls{sym-K}-10$ evaluations if the definition of the $\gls{sym-R-i1-1n}(\gls{sym-r})$ eigenfunction were modified to use a scale factor based on $\gls{sym-a-1}$ rather than $\gls{sym-a-2}$, although this enhancement is not pursued here to maintain consistency with previous work).
