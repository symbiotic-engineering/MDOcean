...
\newcommand{\ZmkDefn}{
    \shortstack{
        $\displaystyle \gls{sym-boldsymbol-mathcal-Z-mk} = \gls{sym-boldsymbol-mathcal-Z-km}^{\\text{top}} = $
        \\
...
\newcommand{\RintTwoJOne}{
    ...
    \frac
    {
        ...
        -\gls{sym-a}_{out}\,\gls{sym-mathrm-K-1} (\gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-out})
    }
    {
        \gls{sym-vec-lambda}_{\gls{sym-m}} \,\gls{sym-mathrm-K-0} (\gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-2})
    }
    $
}
...
\begin{alignat}{5}\label{eq:meem-c-derivation}
    \gls{sym-A-h} + \frac{i\gls{sym-B-h}}{\gls{sym-omega}}&=\gls{sym-rho} \gls{sym-h}^3  \int_{\gls{sym-theta}=0}^{2\gls{sym-pi}}
...
\begin{equation}\label{eq:hydro}
    \gls{sym-A-h} + \frac{i\gls{sym-B-h}}{\gls{sym-omega}}=2\gls{sym-pi} \gls{sym-rho} \gls{sym-h}^3 (\gls{sym-c-0} + \gls{sym-vec-c}\cdot\gls{sym-vec-x}) =2\gls{sym-pi} \gls{sym-rho} \gls{sym-h}^3 (\gls{sym-c-0} + \gls{sym-vec-c} \cdot \gls{sym-A}^{-1} \gls{sym-vec-b})
\end{equation}
...
\begin{equation}\label{eq:gamma-K}
   ...
   \angle \gls{sym-gamma} = -\frac{\gls{sym-pi}}{2} + \angle\frac{ \gls{sym-B-k-0-e}}{\gls{sym-textrm-H-0-1} (\gls{sym-m-0} \gls{sym-a-2})},\quad
...
\end{equation}
where $\gls{sym-g}$ is the acceleration due to gravity, $\gls{sym-rho-w}$ is the density of water, $\gls{sym-m-0}$ is the wavenumber, $\gls{sym-V-g}$ is the finite depth group velocity, $\gls{sym-textrm-H-0-1}$ is the zeroth-order Hankel function of the first kind, and $\gls{sym-A-w}$ is the waterplane area \citep{newman}.
...
Since the largest relevant value of $\gls{sym-m-0}\gls{sym-h}$ depends on the site rather than on the WEC design, it is not possible to add geometric constraints to prevent overflow as it was above.
...
The asymptotic approximation is accurate for values of $\gls{sym-m-0}\gls{sym-h}/\textrm{acosh(realmax})$ as low as $10^{-2}$, though it is only applied for values above $\frac{1}{2}$.
