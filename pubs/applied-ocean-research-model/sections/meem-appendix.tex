...
\begin{table}[htbp]
 ...
 & & $\gls{sym-vec-C-1n-i1-2}$& $\gls{sym-vec-C-1m-i2-2}$& $\gls{sym-vec-C-2m-i2-2}$&$\gls{sym-vec-B-k-e}$\\\hline
          &size&  N&  M&  M& K\\ \hline \hline
          \shortstack{$\gls{sym-phi-i1}=\gls{sym-phi-i2}$ \\ at $\gls{sym-r}=\gls{sym-a-1}$}&N&  $(\gls{sym-h}-\gls{sym-d-1})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-1n-i1})$&  $-\gls{sym-boldsymbol-mathcal-Z-nm}\odot 1_{N1}\gls{sym-vec-R-1m-i2}$&  $-\gls{sym-boldsymbol-mathcal-Z-nm}\odot 1_{N1}\gls{sym-vec-R-2m-i2}$& $0_{NK}$\\ \hline
