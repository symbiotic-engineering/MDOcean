...
Consider a generic function $\gls{sym-Y}(x)$ expressed as a series with coefficients $\gls{sym-alpha}$ and basis functions $e(x)$: $\gls{sym-Y}(x)=\sum_i \gls{sym-alpha-i} \gls{sym-e-i}(x)$.
If $\gls{sym-e-j}(x)$ is orthogonal to $\gls{sym-e-i}(x)$ from $x = \gls{sym-a}$ to $\gls{sym-b}$, then:
...
\begin{equation}\label{eq:hydro}
    \gls{sym-A-h} + \frac{i\gls{sym-B-h}}{\gls{sym-omega}}=2\gls{sym-pi} \gls{sym-rho} \gls{sym-h}^3 (\gls{sym-c-0} + \gls{sym-vec-c}\cdot\gls{sym-vec-x}) =2\gls{sym-pi} \gls{sym-rho} \gls{sym-h}^3 (\gls{sym-c-0} + \gls{sym-vec-c} \cdot \gls{sym-mathbf-A}^{-1} \gls{sym-vec-b})
\end{equation}
