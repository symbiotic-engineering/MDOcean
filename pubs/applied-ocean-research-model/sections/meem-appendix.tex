...
As introduced in \Cref{sec:hydro-meem}, the hydrodynamic coefficients are computed semi-analytically using MEEM.
...
\Cref{fig:meem-regions} illustrates the regions and dimensions: two internal regions \textit{i1} and \textit{i2}, and an external region \textit{e} extending to infinity.
...
\begin{table}[htbp]
    ...
    \expanded{
        \noexpand\tabular{\TableColumnSpecs{aor:meem-eigenfunctions}}
         ...
         Homog. potential $\gls{sym-phi-h}(\gls{sym-r},\gls{sym-z})$&  $\displaystyle\sum_{\gls{sym-n}} \gls{sym-C-1n-i1} \gls{sym-R-1n-i1}(\gls{sym-r}) \gls{sym-Z-n-i1}(\gls{sym-z})$&  $\displaystyle\sum_{\gls{sym-m}} \left(\gls{sym-C-1m-i2} \gls{sym-R-1m-i2}(\gls{sym-r}) + \gls{sym-C-2m-i2} \gls{sym-R-2m-i2}(\gls{sym-r}) \right) \gls{sym-Z-m-i2}(\gls{sym-z})$& $\displaystyle\sum_{\gls{sym-k-index}} \gls{sym-B-k-e} \gls{sym-Lambda-k}(\gls{sym-r}) \gls{sym-Z-k-e}(\gls{sym-z})$\\ \hline
         Partic. potential $\gls{sym-phi-p}(\gls{sym-r},\gls{sym-z})$&  $\displaystyle\frac{1}{2(\gls{sym-h}-\gls{sym-d-1})}\left[ (\gls{sym-z}+\gls{sym-h})^2 - \frac{\gls{sym-r}^2}{2}\right] $&  $\displaystyle\frac{1}{2(\gls{sym-h}-\gls{sym-d-2})}\left[ (\gls{sym-z}+\gls{sym-h})^2 - \frac{\gls{sym-r}^2}{2}\right]$& $0$\\ \hline
         Radial eigen-function $\gls{sym-R}(\gls{sym-r})$&  $\gls{sym-R-1n-i1}(\gls{sym-r}) = \begin{cases}
            \frac{1}{2} &  \gls{sym-n}=0 \\[1em]   %%% <--- here
            \frac{\gls{sym-mathrm-I-0}(\gls{sym-lambda}_{\gls{sym-n}}^{i1}\gls{sym-r})}{\gls{sym-mathrm-I-0}(\gls{sym-lambda}_{\gls{sym-n}}^{i1}\gls{sym-a}_{2})} & \gls{sym-n} \ge 1
        \end{cases} $&  \shortstack{$\gls{sym-R-1m-i2}(\gls{sym-r}) = \begin{cases}
            \frac{1}{2} &  \gls{sym-m}=0 \\[1em]   %%% <--- here
            \frac{\gls{sym-mathrm-I-0}(\gls{sym-lambda}_{\gls{sym-m}}^{i2}\gls{sym-r})}{\gls{sym-mathrm-I-0}(\gls{sym-lambda}_{\gls{sym-m}}^{i2}\gls{sym-a}_{2})} & \gls{sym-m} \ge 1
        \end{cases}$   \\ $\gls{sym-R-2m-i2}(\gls{sym-r}) = \begin{cases}
           \frac{1}{2}\ln(\frac{\gls{sym-r}}{\gls{sym-a-2}}) &  \gls{sym-m} = 0 \\
        \frac{\gls{sym-mathrm-K-0}(\gls{sym-lambda}_{\gls{sym-m}}^{i2}\gls{sym-r})}{\gls{sym-mathrm-K-0}(\gls{sym-lambda}_{\gls{sym-m}}^{i2}\gls{sym-a}_{2})} & \gls{sym-m} \ge 1
        \end{cases}$}& $\gls{sym-Lambda-k}(\gls{sym-r}) = \begin{cases}
           \frac{\mathrm{\gls{sym-H}}_0^{1}(\gls{sym-m}_0r)}{\mathrm{\gls{sym-H}}_0^{1}(\gls{sym-m}_0a_2)} & \gls{sym-k-index} = 0 \\[1em]
          \frac{\gls{sym-mathrm-K-0}(\gls{sym-m}_kr)}{\gls{sym-mathrm-K-0}(\gls{sym-m}_ka_2)} &  \gls{sym-k-index} \ge 1
        \end{cases}$\\ \hline
 Vertical eigen-function $\gls{sym-Z}(\gls{sym-z})$& $\gls{sym-Z-n-i1}(\gls{sym-z}) = \begin{cases}
           1 & \gls{sym-n}=0 \\[1em]   %%% <--- here
           \sqrt{2}\cos(\gls{sym-lambda}_{\gls{sym-n}}^{i1}(\gls{sym-z}+\gls{sym-h})) & \gls{sym-n} \ge 1
        \end{cases}$& $\gls{sym-Z-m-i2}(\gls{sym-z}) = \begin{cases}
           1 & \gls{sym-m}=0 \\[1em]   %%% <--- here
           \sqrt{2}\cos(\gls{sym-lambda}_{\gls{sym-m}}^{i2}(\gls{sym-z}+\gls{sym-h})) & \gls{sym-m} \ge 1
        \end{cases}$&$    \gls{sym-Z}^{e}_{\gls{sym-k-index}}(\gls{sym-z}) = \begin{cases}
           \gls{sym-N-0}^{-\frac{1}{2}}\cosh( \gls{sym-m-0}(\gls{sym-z}+\gls{sym-h})) &  \gls{sym-k-index}=0 \\[1em]   %%% <--- here
           \gls{sym-N-k}^{-\frac{1}{2}}\cos( \gls{sym-m-k}(\gls{sym-z}+\gls{sym-h})) &  \gls{sym-k-index} \ge 1
        \end{cases}$\\ \hline
 Eigen-value& $\displaystyle \gls{sym-lambda-n-i1} = \frac{n\gls{sym-pi}}{\gls{sym-h}-d_{1}},  \gls{sym-n} \geq 1$& $\displaystyle \gls{sym-lambda-m-i2} = \frac{m\gls{sym-pi}}{\gls{sym-h}-d_{2}}, \gls{sym-m} \geq 1$&
 $\displaystyle \begin{cases} \gls{sym-m-0} \tanh(\gls{sym-m}_0h)= \gls{sym-omega}^2/\gls{sym-g}, & \gls{sym-k-index}=0 \\ \gls{sym-m-k} \tan(\gls{sym-m}_kh) = -\gls{sym-omega}^2/\gls{sym-g}, & \gls{sym-k-index} \geq 1\\ \end{cases} $\\\hline
    \endtabular
...
In \Cref{tab:MEEM-eigenfunctions},  $\textrm{\gls{sym-I}}_0$,  $\textrm{\gls{sym-K}}_0$, and $\textrm{\gls{sym-H}}_0^1$ are different Bessel functions of order zero; 1M and 2M mean body 1 and 2 (spar and float) are moving respectively, while 1S and 2S mean each is stationary; and the $\gls{sym-N-k}$ expression is defined as:
...
\newcommand{\RintTwoDefn}{
    $\displaystyle \gls{sym-boldsymbol-mathcal-R-2m} = \int\limits_{\gls{sym-a-in}}^{\gls{sym-a-out}} \gls{sym-vec-R}_{2m}(\gls{sym-r})rdr$
}

\newcommand{\ZmnDefn}{
    \shortstack{
        $\displaystyle \gls{sym-boldsymbol-mathcal-Z-nm} = \gls{sym-boldsymbol-mathcal-Z-mn}^T =$
        \\
        $\displaystyle \int_{-\gls{sym-h}}^{-\gls{sym-d-1}} \gls{sym-vec-Z-n}^{i1 ~\gls{sym-T}}\gls{sym-vec-Z-m}^{i2} dz$
    }
}
\newcommand{\ZmkDefn}{
    \shortstack{
        $\displaystyle \gls{sym-boldsymbol-mathcal-Z-mk} = \gls{sym-boldsymbol-mathcal-Z-km}^T = $
        \\
        $\displaystyle \int_{-\gls{sym-h}}^{-\gls{sym-d-2}} \gls{sym-vec-Z-m}^{i2 ~\gls{sym-T}}\vec{\gls{sym-Z}}_{\gls{sym-k-index}}^{e} dz $
    }
}
...
\newcommand{\RintOneJOne}{
    $\displaystyle
        \frac
            {\gls{sym-a-out} \gls{sym-mathrm-I-1}(\gls{sym-vec-lambda}_{\gls{sym-j}} \gls{sym-a-out}) - \gls{sym-a}_{in}\gls{sym-mathrm-I-1}(\gls{sym-vec-lambda}_{\gls{sym-j}} \gls{sym-a-in})}
...
\newcommand{\RintTwoJOne}{
    ...
    \frac
    {
        \gls{sym-a}_{in}\,\gls{sym-mathrm-K-1} (
                \gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-in} )
        -\gls{sym-a}_{out}\,{\mathrm{\gls{sym-K}}}_1(
                \gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-out} )
    }
    {
        \gls{sym-vec-lambda}_{\gls{sym-m}} \,{\mathrm{\gls{sym-K}}}_0 (
                \gls{sym-vec-lambda}_{\gls{sym-m}} \gls{sym-a-2} )
    }
    $
}
...
\newcommand{\ZnZeroMOne}{
    $\displaystyle
     \frac
                {\sqrt{2}\sin\left(
				\gls{sym-vec-lambda}_{\gls{sym-m}} (\gls{sym-h} - \gls{sym-d-1})
			\right)}
                { \gls{sym-vec-lambda}_{\gls{sym-m}}}
    $
}
...
\newcommand{\ZnOneMZero}{
    $\displaystyle 0
     ...
     %\frac
      %          {\sqrt{2}\sin(\gls{sym-vec-lambda}_{\gls{sym-n}}^T (\gls{sym-h} - \gls{sym-d-1}))}
        %        { \gls{sym-vec-lambda}_{\gls{sym-n}}^T}
    $
}
...
\frac{
            ...
            {
                    1 - \left(
                                    \frac{\gls{sym-vec-lambda}_{\gls{sym-n}}^T}{  \gls{sym-vec-lambda}_{\gls{sym-m}}}
                        \right)^2
            }
\frac{
            \sin\left(
...
,&  \gls{sym-lambda}_{\gls{sym-n}} \neq \gls{sym-lambda-m}\\
\gls{sym-h} -\gls{sym-d-1}, & \gls{sym-lambda}_{\gls{sym-n}} = \gls{sym-lambda-m}
\end{cases}
$
% this more complicated formula is only needed for multi-region
% \shortstack{
...
%     \frac
%         {\sin\left(
% 		(\gls{sym-vec-lambda}_{\gls{sym-n}}^T + \gls{sym-vec-lambda}_{\gls{sym-m}})(\gls{sym-h}-\gls{sym-d-1})
% 	\right)}
%         {\gls{sym-vec-lambda}_{\gls{sym-n}}^T + \gls{sym-vec-lambda}_{\gls{sym-m}}}
%     + \gls{sym-Z}_{nm}^-$
%     \\
...
%         \\
%         \gls{sym-h}-\gls{sym-d-1},
%         & \gls{sym-lambda}_{\gls{sym-n}} = \gls{sym-lambda-m}
%     \end{cases}
%   $
%  }
}
...
\newcommand{\ZmOneKZero}{
    $\displaystyle \frac{\sqrt{2}~\gls{sym-m-0} (-1)^{\vec{\gls{sym-m}}} \sinh(\gls{sym-m-0}(\gls{sym-h}-\gls{sym-d-2}))}{\sqrt{\gls{sym-N-0}} \left(\gls{sym-m-0}^2+{(\gls{sym-vec-lambda}_{\gls{sym-m}}^T)}^2\right)}$
}

\newcommand{\ZkOneMZero}{
    $\displaystyle\frac{
                        \sin(\vec{\gls{sym-m}}_{\gls{sym-k-index}}(\gls{sym-h}-\gls{sym-d-2}))
                        }
                        {
                        \vec{\gls{sym-m}}_{\gls{sym-k-index}}
...
\newcommand{\ZkOneMOne}{
    $\displaystyle
    \begin{cases}
            \frac{1}{\sqrt{2\vec{\gls{sym-N-k}}}}
            \left(
                \frac
                    {
                        \sin\left(
                            (\gls{sym-h}-\gls{sym-d-2})
                            (\vec{\gls{sym-m}}_{\gls{sym-k-index}} + \gls{sym-vec-lambda}_{\gls{sym-m}}^T)
                        \right)
                    }
                    {
                        \vec{\gls{sym-m}}_{\gls{sym-k-index}}
                        + \gls{sym-vec-lambda}_{\gls{sym-m}}^T
                    }
            + \right.
                 \\
                \left.~~~~~~~~~
                \frac
                    {\sin\left((\gls{sym-h}-\gls{sym-d-2})(\vec{\gls{sym-m}}_{\gls{sym-k-index}}- \gls{sym-vec-lambda}_{\gls{sym-m}}^T)\right)}
                    {\vec{\gls{sym-m}}_{\gls{sym-k-index}}- \gls{sym-vec-lambda}_{\gls{sym-m}}^T}
            \right),
        & |\gls{sym-m-k}| \neq \gls{sym-lambda-m}
        \\
        \frac{\gls{sym-h}-\gls{sym-d-2}}{2},
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|P{0.25\linewidth}|c|} \hline
        \multicolumn{2}{|c|}{Integral}          & $\gls{sym-m},\gls{sym-j}=0$           & $\gls{sym-m},\gls{sym-j}\geq1$            \\ \hline
        \multicolumn{2}{|c|}{\RintOneDefn}      & \RintOneJzero     & \RintOneJOne          \\ \hline
        \multicolumn{2}{|c|}{\RintTwoDefn}      & \RintTwoJzero     & \RintTwoJOne          \\ \hline
        \multirow{2}{*}{\ZmnDefn}   & $\gls{sym-n}=0$     & \ZnZeroMZero      & \ZnZeroMOne           \\ \cline{2-4}
                                    & $\gls{sym-n}\geq1$  & \ZnOneMZero       & \ZnOneMOne            \\ \hline
        \multirow{2}{*}{\ZmkDefn}   & $\gls{sym-k-index}=0$     & \ZmZeroKZero      & \ZmOneKZero           \\ \cline{2-4}
                                    & $\gls{sym-k-index}\geq1$  & \ZkOneMZero       & \ZkOneMOne            \\ \hline
    \end{tabular}
\end{table}
...
Assuming truncation to $\gls{sym-N}$ terms in \textit{i1}, $\gls{sym-M}$ terms in \textit{i2}, and $\gls{sym-K}$ terms in \textit{e}, the total number of eigencoefficients to solve for is $\gls{sym-N}+2M+\gls{sym-K}$.
...
As-is, this is not enough equations ($4 < \gls{sym-N}+2M+\gls{sym-K}$).
...
Consider a generic function $\gls{sym-Y}(\gls{sym-x})$ expressed as a series with coefficients $\gls{sym-alpha}$ and basis functions $e(\gls{sym-x})$: $\gls{sym-Y}(\gls{sym-x})=\sum_i \gls{sym-alpha-i} \gls{sym-e-i}(\gls{sym-x})$.
...
In the current hydrodynamics problem, the basis functions are the vertical eigenfunctions $\gls{sym-Z-n-i1}$, $\gls{sym-Z-m-i2}$, and $\gls{sym-Z}_{\gls{sym-k-index}}^{e}$.
Orthogonality of each eigenfunction can be verified with the inner product.
In the first region, for example, $<\gls{sym-Z-n-1-i1},\gls{sym-Z-n-2-i1}>=\gls{sym-delta-n-1n-2}$.
...
Once orthogonality is applied, the matching equations create a linear system $A\gls{sym-vec-x}=\gls{sym-vec-b}$ where $\gls{sym-A}$ is a complex sparse $(\gls{sym-N}+2M+\gls{sym-K})$x$(\gls{sym-N}+2M+\gls{sym-K})$ square matrix corresponding to the homogeneous case, $\gls{sym-vec-x}=[\gls{sym-vec-C-1n-i1}, \gls{sym-vec-C-1m-i2}, \gls{sym-vec-C-2m-i2}, \vec{\gls{sym-B}_{\gls{sym-k-index}}^{e}}]$ is the complex eigencoefficient vector, and $\gls{sym-vec-b}$ is the real boundary condition vector corresponding to the particular case.
...
They are written in compact notation using row vectors of basis functions, so $\gls{sym-vec-R-1n-i1}=[\gls{sym-R-10-i1}, \gls{sym-R-11-i1}, ..., \gls{sym-R-1-N-1-i1}]$ and so on.
...
\begin{table}[htbp]
 ...
 & & $\gls{sym-vec-C-1n-i1-2}$& $\gls{sym-vec-C-1m-i2-2}$& $\gls{sym-vec-C-2m-i2-2}$&$\gls{sym-vec-B-k-e-2}$\\\hline
          &size&  N&  M&  M& K\\ \hline \hline
          \shortstack{$\gls{sym-phi-i1}=\gls{sym-phi-i2}$ \\ at $\gls{sym-r}=\gls{sym-a-1}$}&N&  $(\gls{sym-h}-\gls{sym-d-1})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-1n-i1-2})$&  $-\gls{sym-boldsymbol-mathcal-Z-nm}\odot 1_{N1}\gls{sym-vec-R-1m-i2}$&  $-\gls{sym-boldsymbol-mathcal-Z-nm}\odot 1_{N1}\gls{sym-vec-R-2m-i2}$& $0_{NK}$\\ \hline
          \shortstack{$\gls{sym-phi-i2}=\gls{sym-phi-e}$ \\ at $\gls{sym-r}=\gls{sym-a-2}$}&M&  $0_{MN}$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-1m-i2})$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-2m-i2})$& $-\gls{sym-boldsymbol-mathcal-Z-mk}\odot 1_{M1}\vec{\Lambda}_{\gls{sym-k-index}}$\\ \hline
          \shortstack{$\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-i1}=\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-i2}$ \\ at $\gls{sym-r}=\gls{sym-a-1}$}&M&  $- \gls{sym-boldsymbol-mathcal-Z-mn} \odot 1_{M1} \frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-1n-i1-2}$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-1m-i2})$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-2m-i2})$& $0_{MK}$\\ \hline
          \shortstack{$\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-i2}=\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-e}$ \\ at $\gls{sym-r}=\gls{sym-a-2}$}&K&  $0_{KN}$&  $-\gls{sym-boldsymbol-mathcal-Z-km} \odot 1_{K1}\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-1m-i2}$&  $-\gls{sym-boldsymbol-mathcal-Z-km}\odot 1_{K1}\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-2m-i2}$& $\gls{sym-h}~\gls{sym-mathrm-diag}(\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-Lambda-k})$\\ \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|} \hline
         N& $\displaystyle
\int_{-\gls{sym-h}}^{-\gls{sym-d-1}} (\gls{sym-phi-p}^{i2} - \gls{sym-phi-p}^{i1})\gls{sym-vec-Z-n}^{i1~\gls{sym-T}} dz
$ \\ \hline
         M& $\displaystyle-
\int_{-\gls{sym-h}}^{-\gls{sym-d-2}} \gls{sym-phi-p}^{i2} \gls{sym-vec-Z-m}^{i2~\gls{sym-T}} dz
$\\ \hline
         M& $\displaystyle
\int_{-\gls{sym-h}}^{-\gls{sym-d-1}} \frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-p}^{i1}\gls{sym-vec-Z-m}^{i2~\gls{sym-T}} dz - \int_{-\gls{sym-h}}^{-\gls{sym-d-2}}\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-p}^{i2}\gls{sym-vec-Z-m}^{i2~\gls{sym-T}} dz
$\\ \hline
         K& $\displaystyle
\int_{-\gls{sym-h}}^{-\gls{sym-d-2}} \frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-p}^{i2} \vec{\gls{sym-Z}}_{\gls{sym-k-index}}^{e~\gls{sym-T}} dz
$\\ \hline
    \end{tabular}
\end{table}

The dense blocks contain coupling integrals $\boldsymbol{\mathcal{\gls{sym-Z}}}$ of the vertical eigenfunctions.
...
\begin{alignat}{5}\label{eq:meem-c-derivation}
    A_h + \frac{iB_h}{\omega}&=\rho h^3  \int_{\theta=0}^{2\pi}
    & \int_{r=a_{in}}^{a_{out}}
    &\phi(r,z) \frac{\partial \phi(r,z)}{\partial z}
    r dr d\theta  \nonumber \\
    %
    %
    &= 2\pi \rho h^3
  ...
  & & &  \left(\frac{\partial \phi_p(r,z)}{\partial z}
    ...
    \right] \nonumber
\end{alignat}
where the generic indices $\gls{sym-j-1}$ and $\gls{sym-j-2}$ are used to represent $\gls{sym-n}$ or $\gls{sym-m}$ depending on the region.
...
\begin{tabular}{|l|c|c|c|c|} \hline
          Body&N&  M&  M& K\\ \hline
          Float&$0_{1N}$&  $\gls{sym-vec-Z-m}(\gls{sym-z}=-d) \odot\gls{sym-boldsymbol-mathcal-R-1m}$&  $\gls{sym-vec-Z-m}(\gls{sym-z}=-d) \odot\gls{sym-boldsymbol-mathcal-R-2m}$& $0_{1K}$\\ \hline
          Spar& $\gls{sym-vec-Z-n}(\gls{sym-z}=-d) \odot\gls{sym-boldsymbol-mathcal-R-1n}$& $0_{1M}$& $0_{1M}$&$0_{1K}$\\\hline
    \end{tabular}
    \end{table}
...
Using the A-matrix and b- and c-vectors, the MEEM solution directly calculates radiation coefficients $\gls{sym-B-h}$ and $\gls{sym-A-h}$.
...
\begin{equation}\label{eq:gamma-K}
   ...
   \angle \gamma = -\frac{\pi}{2} + \angle\frac{ B_{k=0}^e}{\textrm{H}_0^{1}(m_0 a_2)},\quad
    K_h       = \rho_w g \underbrace{\frac{\pi}{4} (D_f^2 - D_s^2)}_{A_{w,f}},\quad  % hydrostatic stiffness
    K_s = \rho_w g \underbrace{\frac{\pi}{4} D_s^2}_{A_{w,s}}
\end{equation}
where $\gls{sym-g}$ is the acceleration due to gravity, $\gls{sym-rho-w}$ is the density of water, $\gls{sym-m-0}$ is the wavenumber, $\gls{sym-V-g}$ is the finite depth group velocity, $\textrm{\gls{sym-H}}_0^1$ is the zeroth-order Hankel function of the first kind, and $\gls{sym-A-w}$ is the waterplane area \citep{newman}.
This method of calculating excitation from damping is the well-known Haskind relation.
Note that while the excitation magnitude $|\gls{sym-gamma}|$ depends on the radiation damping $\gls{sym-B-h}$ which in turn depends on all the inner region eigencoefficients ($\vec{\gls{sym-C}}_{\gls{sym-m}}^{i2}$ for float excitation and $\gls{sym-vec-C-1n-i1-2}$ for the spar excitation), the excitation phase $\angle\gls{sym-gamma}$ depends only on the first exterior eigencoefficient, $\gls{sym-B-k-0-e}$.

\subsection{Verification and Validation}

Hydro coefficient results are verified by comparing to a benchmark shallow-water concentric-cylinder MEEM solution in \citet{chau_inertia_2012}.
...
Previously in \Cref{sec:dynamic-validation}, the N=M=K=11 case was compared to WAMIT results for RM3 in deep water.
...
As $\gls{sym-N},\gls{sym-M},K\rightarrow\infty$, matching quality improves, and hydro coefficients converge toward their true values.
Previous MEEM papers use $\gls{sym-N}=\gls{sym-M}=\gls{sym-K}=50$ to obtain 4-digit matching accuracy without elaborating on convergence properties \citep{chau_inertia_2012}.
...
Hydro coefficient convergence depends on the geometry: the benchmark shallow-water geometry of \citep{chau_inertia_2012} converges to within 0.25\% with only $\gls{sym-N}=\gls{sym-M}=\gls{sym-K}=4$, but RM3 requires $\gls{sym-N}=\gls{sym-M}=\gls{sym-K}>10$, shown in \Cref{fig:meem-convergence}.
...
\begin{figure}[htbp]
    ...
    \caption{Convergence for $\gls{sym-N}=\gls{sym-M}=\gls{sym-K}=(5,10,20,30)$ for RM3}
...
To prevent overflow, each fluid region must exceed a minimum height $ \gls{sym-Delta} \gls{sym-z}_{\text{min}}$ that is proportional to the region diameter $\gls{sym-D}$ for both the spar ($\gls{sym-Delta} \gls{sym-z-s}, \gls{sym-D-s}$) and the float ($\gls{sym-Delta} \gls{sym-z-f}, \gls{sym-D-f}$), with variables shown in \Cref{fig:meem-geom}.
The constant of proportionality between $\gls{sym-Delta} \gls{sym-z}_{\text{min}}$ and $\gls{sym-D}$ is a function of the number of harmonics $\gls{sym-N}_{\text{harmonics}}$ used in that region ($\gls{sym-N}$ for spar, $\gls{sym-M}$ for float), as well as the maximum argument to the \texttt{besseli} function in MATLAB before overflow, $\gls{sym-chi}_{\text{max}}$:
\begin{equation}\label{eq:delta-z-min}
    \frac{\Delta z_{min}}{D} = \frac{\pi N_{harmonics}}{2\chi_{max}} \approx \frac{N_{harmonics}}{446}.
\end{equation}
By trial, $\gls{sym-chi}_{\text{max}}$ is found to be $\approx 700.5$, close to the theoretical value of \texttt{log(realmax)} $\approx 709.8$ for exponential scaling.
Since $\gls{sym-N}_{\text{harmonics}}=10$ gives adequate convergence for most geometries, this condition is trivially satisfied for nearly all floating bodies of practical relevance, but must still be added as a constraint to prevent the optimizer from exploiting the numerical divergence.
...
Likewise, the vertical eigenfunction $\gls{sym-Z-k-e}$ for $\gls{sym-k-index}=0$ contains the $\cosh$ and $\sinh$ functions, which diverge for large values of $\gls{sym-m}_0h$ (high frequencies or deep water).
Since the largest relevant value of $\gls{sym-m}_0h$ depends on the site rather than on the WEC design, it is not possible to add geometric constraints to prevent overflow as it was above.
...
\end{equation}
Plugging this into the first element of the bottom block of the b-vector (position $\gls{sym-N}+2M+1$) results in
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Asymptotic b-vector for large $\gls{sym-m}_0h$}
    \label{fig:meem-b-limit}
\end{figure}

\Cref{fig:meem-b-limit} shows the asymptotic approximation as well as the standard expression for the relevant b-vector entry as a function of nondimensional ratio $\gls{sym-m}_0h/\textrm{acosh(realmax})$.
With the standard expression, values for inputs above $\frac{1}{2}$ become \texttt{NaN}, which poses problems in deep water.
With the asymptotic approximation, we are able to avoid \texttt{NaN}s for the zone $\frac{1}{2}<\gls{sym-m}_0h/\textrm{acosh(realmax})<\frac{\gls{sym-h}}{\gls{sym-d-2}}$.
Above this threshold, \texttt{NaN}s occur even with the approximation, and the value in the b-vector is set to zero.
This introduces negligible error because in this range, the magnitude of the true value is less than $10^{-300}$ for the nominal RM3 geometry.
The asymptotic approximation is accurate for values of $\gls{sym-m}_0h/\textrm{acosh(realmax})$ as low as $10^{-2}$, though it is only applied for values above $\frac{1}{2}$.
...
Bounds of $180^\textrm{o}\cdot[\gls{sym-k-index}-\frac{1}{2}, \gls{sym-k-index}]$ are placed on $\gls{sym-m}_kh$ in a root-finding algorithm to ensure the $\gls{sym-k-index}$th root is identified.
Without these bounds, sometimes the \texttt{fsolve} solver would wrongly converge to values where the residual approaches $+\infty$ from one side and $-\infty$ from the other (asymptotes of $\tan()$) rather than true zeroes.
...
Then $3N+8M+2K-11$ Bessel functions must be evaluated for the radial terms of the A-matrix.
(Note that this would reduce to $2N+8M+2K-10$ evaluations if the definition of the $\gls{sym-R-i1-1n}(\gls{sym-r})$ eigenfunction were modified to use a scale factor based on $\gls{sym-a-1}$ rather than $\gls{sym-a-2}$, although this enhancement is not pursued here to maintain consistency with previous work).
...
Linear solves scale almost cubically with matrix size, so this step scales with $(\gls{sym-N}+2M+\gls{sym-K})^3$.
