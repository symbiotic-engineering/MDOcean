...
\begin{table}[htbp]
    ...
    Partic. potential $\gls{sym-phi-p}(\gls{sym-r},\gls{sym-z})$
        ...
        \\
        \hline 
    Radial eigen-function $\gls{sym-R}(\gls{sym-r})$
        &  $\gls{sym-R-1n-i1}(\gls{sym-r})
            =
            \begin{cases}
                \frac12
...
\begin{equation}
    ...
    f_k =
    \begin{cases}
        \sinh(2\gls{sym-m-0}\gls{sym-h}), & \gls{sym-k}=0 \\ \sin(2\gls{sym-m-k}\gls{sym-h}), & \gls{sym-k}\geq1
    \end{cases}
\end{equation}
...
\begin{equation}\label{eq:gamma-K}
   ...
   \angle \gls{sym-gamma} = -\frac{\gls{sym-pi}}{2} + \angle\frac{ \gls{sym-B-k-0-e}}{\textrm{\gls{sym-H}}_0^{1}(\gls{sym-m-0} \gls{sym-a-2})},\quad
