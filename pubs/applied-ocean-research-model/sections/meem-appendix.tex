...
In the first region, for example, $<Z_{n_1}^{i_1},Z_{n_2}^{i_1}>=\delta_{n_1 n_2}$.
...
\begin{table}[htbp]
 ...
 & & $\gls{sym-vec-C-1n-i1-2}$& $\gls{sym-vec-C-1m-i2-2}$& $\gls{sym-vec-C-2m-i2-2}$&$\gls{sym-vec-B-k-e}$\\\hline
          ...
          \shortstack{$\gls{sym-phi-i1}=\gls{sym-phi-i2}$ \\ at $\gls{sym-r}=\gls{sym-a-1}$}&N&  $(\gls{sym-h}-\gls{sym-d-1})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-1n-i1})$&  $-\gls{sym-boldsymbol-mathcal-Z}_{nm}\odot 1_{N1}\gls{sym-vec-R-1m-i2}$&  $-\gls{sym-boldsymbol-mathcal-Z}_{nm}\odot 1_{N1}\gls{sym-vec-R-2m-i2}$& $0_{NK}$\\ \hline
          \shortstack{$\gls{sym-phi-i2}=\gls{sym-phi-e}$ \\ at $\gls{sym-r}=\gls{sym-a-2}$}&M&  $0_{MN}$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-1m-i2})$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-2m-i2})$& $-\gls{sym-boldsymbol-mathcal-Z}_{mk}\odot 1_{M1}\vec{\Lambda}_{\gls{sym-k-index}}$\\ \hline
          \shortstack{$\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-i1}=\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-i2}$ \\ at $\gls{sym-r}=\gls{sym-a-1}$}&M&  $- \gls{sym-boldsymbol-mathcal-Z}_{mn} \odot 1_{M1} \frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-1n-i1}$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-1m-i2})$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-2m-i2})$& $0_{MK}$\\ \hline
          \shortstack{$\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-i2}=\frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-e}$ \\ at $\gls{sym-r}=\gls{sym-a-2}$}&K&  $0_{KN}$&  $-\gls{sym-boldsymbol-mathcal-Z}_{km} \odot 1_{K1}\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-1m-i2}$&  $-\gls{sym-boldsymbol-mathcal-Z}_{km}\odot 1_{K1}\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-R-2m-i2}$& $\gls{sym-h}~\gls{sym-mathrm-diag}(\frac{\partial}{\partial \gls{sym-r}}\gls{sym-vec-Lambda-k})$\\ \hline
    \end{tabular}
\end{table}
...
\begin{alignat}{5}\label{eq:meem-c-derivation}
  ...
  & & &  \left (\frac{\partial \phi_p (r,z)}{\partial z}
    ...
    \right] \nonumber
\end{alignat}
where the generic indices $j_1$ and $j_2$ are used to represent $\gls{sym-n}$ or $\gls{sym-m}$ depending on the region.
...
\begin{equation}\label{eq:gamma-K}
   ...
   \angle \gls{sym-gamma} = -\frac{\gls{sym-pi}}{2} + \angle\frac{ \gls{sym-B-k-0-e}}{\gls{sym-mathrm-H-0-1} (\gls{sym-m-0} \gls{sym-a-2})},\quad
...
\begin{equation}
\begin{aligned}
    \lim_{\gls{sym-m-0}\gls{sym-h}\rightarrow\infty}\gls{sym-boldsymbol-mathcal-Z}_{m,k=0} &
