...
\begin{table}[htbp]
    ...
    \expanded{
        \noexpand\tabular{\TableColumnSpecs{aor:meem-eigenfunctions}}
         ...
         Homog. potential $\gls{sym-phi-h}(\gls{sym-r},\gls{sym-z})$&  $\displaystyle\sum_{\gls{sym-n}} \gls{sym-C-1n-i1} \gls{sym-R-1n-i1}(\gls{sym-r}) \gls{sym-Z-n-i1}(\gls{sym-z})$&  $\displaystyle\sum_{\gls{sym-m}} \left(\gls{sym-C-1m-i2} \gls{sym-R-1m-i2}(\gls{sym-r}) + \gls{sym-C-2m-i2} \gls{sym-R-2m-i2}(\gls{sym-r}) \right) \gls{sym-Z-m-i2}(\gls{sym-z})$& $\displaystyle\sum_{\gls{sym-k}} \gls{sym-B-k-e} \gls{sym-Lambda-k}(\gls{sym-r}) \gls{sym-Z-k-e}(\gls{sym-z})$\\ \hline
        ...
        \end{cases}$}& $\gls{sym-Lambda-k}(\gls{sym-r}) = \begin{cases}
           \frac{\mathrm{\gls{sym-H}}_0^{1}(\gls{sym-m}_0r)}{\mathrm{\gls{sym-H}}_0^{1}(\gls{sym-m}_0a_2)} & \gls{sym-k} = 0 \\[1em]
          \frac{\gls{sym-mathrm-K-0}(\gls{sym-m}_kr)}{\gls{sym-mathrm-K-0}(\gls{sym-m}_ka_2)} &  \gls{sym-k} \ge 1
        \end{cases}$\\ \hline
 Vertical eigen-function $\gls{sym-Z}(\gls{sym-z})$& $\gls{sym-Z-n-i1}(\gls{sym-z}) = \begin{cases}
        ...
        \end{cases}$&$    \gls{sym-Z}^{e}_{\gls{sym-k}}(\gls{sym-z}) = \begin{cases}
           \gls{sym-N-0}^{-\frac{1}{2}}\cosh( \gls{sym-m-0}(\gls{sym-z}+\gls{sym-h})) &  \gls{sym-k}=0 \\[1em]   %%% <--- here
           \gls{sym-N-k}^{-\frac{1}{2}}\cos( \gls{sym-m-k}(\gls{sym-z}+\gls{sym-h})) &  \gls{sym-k} \ge 1
        \end{cases}$\\ \hline
 Eigen-value& $\displaystyle \gls{sym-lambda}_{\gls{sym-n}}^{i1} = \frac{n\gls{sym-pi}}{\gls{sym-h}-d_{1}},  \gls{sym-n} \geq 1$& $\displaystyle \gls{sym-lambda}_{\gls{sym-m}}^{i2} = \frac{m\gls{sym-pi}}{\gls{sym-h}-d_{2}}, \gls{sym-m} \geq 1$&
 $\displaystyle \begin{cases} \gls{sym-m-0} \tanh(\gls{sym-m}_0h)= \gls{sym-omega}^2/\gls{sym-g}, & \gls{sym-k}=0 \\ \gls{sym-m-k} \tan(\gls{sym-m}_kh) = -\gls{sym-omega}^2/\gls{sym-g}, & \gls{sym-k} \geq 1\\ \end{cases} $\\\hline
    \endtabular
...
\newcommand{\ZmkDefn}{
    \shortstack{
        ...
        \\
        $\displaystyle \int_{-\gls{sym-h}}^{-\gls{sym-d-2}} \gls{sym-vec-Z-m}^{i2 ~\gls{sym-T}}\vec{\gls{sym-Z}}_{\gls{sym-k}}}^{e} dz $
    }
}
...
\newcommand{\ZkOneMZero}{
    $\displaystyle\frac{
                        \sin(\vec{\gls{sym-m}}_{\gls{sym-k}}(\gls{sym-h}-\gls{sym-d-2}))
                        }
                        {
                        \vec{\gls{sym-m}}_{\gls{sym-k}}
...
\newcommand{\ZkOneMOne}{
    ...
    \begin{cases}
                ...
                \frac
                    {
                        \sin\left(
                            ...
                            (\vec{\gls{sym-m}}_{\gls{sym-k}} + \gls{sym-vec-lambda}_{\gls{sym-m}}^T)
                        \right)
                    }
                    {
                        \vec{\gls{sym-m}}_{\gls{sym-k}}
            ...
            + \right.
                ...
                \left.~~~~~~~~~
                \frac
                    {\sin\left((\gls{sym-h}-\gls{sym-d-2})(\vec{\gls{sym-m}}_{\gls{sym-k}}- \gls{sym-vec-lambda}_{\gls{sym-m}}^T)\right)}
                    {\vec{\gls{sym-m}}_{\gls{sym-k}}- \gls{sym-vec-lambda}_{\gls{sym-m}}^T}
            \right),
        & |\gls{sym-m-k}| \neq \gls{sym-lambda-m}
        \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|P{0.25\linewidth}|c|} \hline
        ...
        \multirow{2}{*}{\ZmkDefn}   & $\gls{sym-k}=0$     & \ZmZeroKZero      & \ZmOneKZero           \\ \cline{2-4}
                                    & $\gls{sym-k}\geq1$  & \ZkOneMZero       & \ZkOneMOne            \\ \hline
    \end{tabular}
\end{table}
...
In the current hydrodynamics problem, the basis functions are the vertical eigenfunctions $\gls{sym-Z-n-i1}$, $\gls{sym-Z-m-i2}$, and $\gls{sym-Z}_{\gls{sym-k}}^{e}$.
...
Once orthogonality is applied, the matching equations create a linear system $A\gls{sym-vec-x}=\gls{sym-vec-b}$ where $\gls{sym-A}$ is a complex sparse $(\gls{sym-N}+2M+\gls{sym-K})$x$(\gls{sym-N}+2M+\gls{sym-K})$ square matrix corresponding to the homogeneous case, $\gls{sym-vec-x}=[\gls{sym-vec-C-1n-i1}, \gls{sym-vec-C-1m-i2}, \gls{sym-vec-C-2m-i2}, \vec{\gls{sym-B}_{\gls{sym-k}}^{e}}]$ is the complex eigencoefficient vector, and $\gls{sym-vec-b}$ is the real boundary condition vector corresponding to the particular case.
...
\begin{table}[htbp]
 ...
 & & $\gls{sym-vec-C-1n-i1-2}$& $\gls{sym-vec-C-1m-i2-2}$& $\gls{sym-vec-C-2m-i2-2}$&$\gls{sym-vec-B-k-e-2}$\\\hline
          ...
          \shortstack{$\gls{sym-phi-i2}=\gls{sym-phi-e}$ \\ at $\gls{sym-r}=\gls{sym-a-2}$}&M&  $0_{MN}$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-1m-i2})$&  $(\gls{sym-h}-\gls{sym-d-2})~\gls{sym-mathrm-diag}(\gls{sym-vec-R-2m-i2})$& $-\gls{sym-boldsymbol-mathcal-Z-mk}\odot 1_{M1}\vec{\Lambda}_{\gls{sym-k}}$\\ \hline
...
$\\ \hline
         K& $\displaystyle
\int_{-\gls{sym-h}}^{-\gls{sym-d-2}} \frac{\partial}{\partial \gls{sym-r}}\gls{sym-phi-p}^{i2} \vec{\gls{sym-Z}}_{\gls{sym-k}}^{e~\gls{sym-T}} dz
$\\ \hline
    \end{tabular}
\end{table}
...
Likewise, the vertical eigenfunction $\gls{sym-Z-k-e}$ for $\gls{sym-k}=0$ contains the $\cosh$ and $\sinh$ functions, which diverge for large values of $\gls{sym-m}_0h$ (high frequencies or deep water).
...
Bounds of $180^\textrm{o}\cdot[\gls{sym-k}-\frac{1}{2}, \gls{sym-k}]$ are placed on $\gls{sym-m}_kh$ in a root-finding algorithm to ensure the $\gls{sym-k}$th root is identified.
