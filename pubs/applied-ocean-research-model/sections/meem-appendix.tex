...
Consider a generic function $\gls{sym-Y}(x)$ expressed as a series with coefficients $\gls{sym-alpha-generic}$ and basis functions $e(x)$: $\gls{sym-Y}(x)=\sum_i \gls{sym-alpha-generic}_i \gls{sym-e-i}(x)$.
If $\gls{sym-e-j}(x)$ is orthogonal to $\gls{sym-e-i}(x)$ from $x = \gls{sym-a}$ to $\gls{sym-b}$, then:

\begin{equation}
\begin{aligned}
       & \int\limits_a^b Y (x)e_j (x)dx = (b-a) <Y,e_j>= (b-a)<\sum_i \gls{sym-alpha-generic}_i e_i, e_j> \\ &= (b-a) \sum_i \gls{sym-alpha-generic}_i <e_i,e_j> = (b-a) \sum_i \gls{sym-alpha-generic}_i \delta_{ij} = (b-a) \gls{sym-alpha-generic}_j
\end{aligned}
\end{equation}
