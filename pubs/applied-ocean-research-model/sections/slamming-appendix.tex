...
For operational waves, the vertical dimension $\gls{sym-Delta-z}$ in \Cref{eq:slam-constraint-quadratic} is set as follows.
For float slamming, $\gls{sym-Delta-z} = \gls{sym-T-f-1}$ (rather than $\gls{sym-T-f-2}$):
...
For spar slamming, $\gls{sym-Delta-z} = \gls{sym-T-s} - \gls{sym-h-d}$
