...
\begin{aligned}
    \gls{sym-xi-max-slam} &= \frac{\gls{sym-H}}{2} \cos\gls{sym-theta} + \sqrt{\gls{sym-Delta-z-slam}^2-\left(\frac{\gls{sym-H}}{2}\sin\gls{sym-theta}\right)^2 } \\
    \gls{sym-xi-min-slam} &= \frac{\gls{sym-H}}{2} \cos\gls{sym-theta} - \sqrt{\gls{sym-Delta-z-slam}^2-\left(\frac{\gls{sym-H}}{2}\sin\gls{sym-theta}\right)^2 }
\end{aligned}
\end{equation}
where $\gls{sym-theta}=\angle\gls{sym-hat-xi} + \gls{sym-k-wavenumber}\gls{sym-y}$ must be evaluated with the value of $\gls{sym-y}$ that creates the minimum value of $\gls{sym-xi}_{\text{max},\text{slam}}$ and the maximum value of $\gls{sym-xi}_{\text{min},\text{slam}}$ to ensure lack of slamming across the entire \gls{acr-wec} surface.
...
The value of $\gls{sym-theta}$ that minimizes $\gls{sym-xi-max-slam}$ over the relevant range is
\begin{equation}\label{eq:slamming-limit-small-waves}
\gls{sym-theta} = \gls{sym-pi} - \max\left(
                                            0,~
...
for short wavelengths $(\gls{sym-theta}=\gls{sym-pi})$ and long wavelengths $(\gls{sym-theta}=0)$ respectively.
...
Intuitively, $\gls{sym-theta}$ represents how high up the free surface the bottom of the \gls{acr-wec} is allowed to get before the edge exits the water,
with $\gls{sym-theta}=0$ allowing the body to get all the way to the wave crest (top)
and $\gls{sym-theta}=\gls{sym-pi}$ requiring the float to remain fully below the wave trough (bottom).
...
\begin{equation}
\gls{sym-theta} = \angle\gls{sym-hat-xi} + \frac{\gls{sym-k-wavenumber}\gls{sym-D}}{2}\textrm{sgn}(\gls{sym-pi}-\angle\gls{sym-hat-xi})
\end{equation}
...
as a function of the worst-case phase angle $\gls{sym-theta}$
and the draft to wave amplitude ratio $\gls{sym-Delta-z-slam} /(\gls{sym-H}/2)$.
This reveals that besides increasing the draft, decreasing $\gls{sym-theta}$ can prevent slamming,
for example by decreasing the diameter or adjusting $\angle \gls{sym-xi}$ via control.
If $\gls{sym-Delta-z-slam} <\gls{sym-H}/2|\sin\gls{sym-theta}|$,
...
Comparing the first two subplots for the phase of the float motion $\angle\gls{sym-hat-xi-f}$ and the slamming angle $\gls{sym-theta}$
...
but also adjust $\angle\gls{sym-hat-xi-f}$ to reduce $\gls{sym-theta}$.
\begin{figure}[htbp]
    \centering
