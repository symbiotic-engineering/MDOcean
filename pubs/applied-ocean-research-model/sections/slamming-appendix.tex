In regular waves of height $\gls{sym-H}$, the conditions for slamming can be derived by comparing the vertical position of the bottom of the WEC, $\gls{sym-xi}(\gls{sym-t})-\gls{sym-Delta-z-slam}$, with the wave elevation $\gls{sym-zeta}(\gls{sym-y},\gls{sym-t})$, which depends on the horizontal position coordinate $\gls{sym-y}$.
To prevent slamming, the criteria
\begin{equation}\label{eq:slamming-time-domain}
\begin{aligned}
    \gls{sym-xi}(\gls{sym-t}) - \gls{sym-Delta-z-slam} &< \gls{sym-zeta}(\gls{sym-y},\gls{sym-t}) \\
    |\gls{sym-hat-xi}|\cos(\gls{sym-omega} \gls{sym-t}+\angle \gls{sym-hat-xi}) - \gls{sym-Delta-z-slam} &< \frac{\gls{sym-H}}{2}\cos(\gls{sym-omega} \gls{sym-t} - \gls{sym-k}\gls{sym-y})
\end{aligned}
\end{equation}
...
\begin{equation}
|\gls{sym-hat-xi}|^2 - \gls{sym-H}\cos(\gls{sym-k}\gls{sym-y})|\gls{sym-hat-xi}|\cos\angle\gls{sym-hat-xi} + \gls{sym-H}\sin(\gls{sym-k}\gls{sym-y})|\gls{sym-hat-xi}|\sin\angle\gls{sym-hat-xi} < \gls{sym-Delta-z-slam}^2-\left(\frac{\gls{sym-H}}{2}\right)^2
\end{equation}
...
\begin{equation}\label{eq:slamming-squared}
    \left[ |\gls{sym-hat-xi}|- \frac{\gls{sym-H}}{2}\cos(\angle\gls{sym-hat-xi} + \gls{sym-k}\gls{sym-y})\right]^2 < \gls{sym-Delta-z-slam}^2 - (\gls{sym-H}/2)^2 \sin^2(\angle\gls{sym-hat-xi} + \gls{sym-k}\gls{sym-y})
\end{equation}
...
This expression reduces to the simple limits $\gls{sym-Deltaz-slam} -\gls{sym-H}/2$ and $\gls{sym-Delta-z-slam} +\gls{sym-H}/2$ for short wavelengths $(\gls{sym-theta}=\gls{sym-pi})$ and long wavelengths $(\gls{sym-theta}=0)$ respectively.
...
The widest permissible amplitude range occurs when the WEC is perfectly in-phase with the incident wave ($\angle\gls{sym-hat-xi}=0$) and is small with respect to the wavelength ($kD\approx0$), which yields the limits $\gls{sym-H}/2-\gls{sym-Delta-z-slam} < |\gls{sym-hat-xi}| < \gls{sym-H}/2+\gls{sym-Delta-z-slam}$.
