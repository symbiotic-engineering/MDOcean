...
$\gls{sym-tau-max}$, $\gls{sym-P}_{\gls{sym-tau}\gls{sym-Omega},\text{max}}$, $\gls{sym-xi}_{\gls{sym-f},\text{max}}$, $\gls{sym-xi}_{\gls{sym-s},\text{max}}$, and $\gls{sym-X-PTO-max}$, which we collectively denote as $\gls{sym-L-mu}$.
...
$\frac{\partial \gls{sym-p-avg-VI-opt}}{\partial \gls{sym-L-mu}}$
...
\begin{equation}\label{eq:gamma-opt-summary}
    ...
    & \text{only $\gls{sym-mu}^*$ active} \\
        \left|
            \gls{sym-Gamma-c-mu-star} + \frac{\gls{sym-Gamma-c-nu-star} - \gls{sym-Gamma-c-mu-star}}{d_{\mu^*\nu^*}}
...
\begin{aligned}
    |\gls{sym-Gamma-opt}| &=
    \min \left\{
        |\gls{sym-Gamma-cl-mu-star} (b_\mu)|,  |\Gamma_{int,\mu^*,\nu^*} (b_\mu, b_\nu)|
    \right\}
\end{aligned}
\end{equation}
...
%\end{equation}
%where $\gls{sym-tau-max}$ is the maximum permissible generator torque and $\gls{sym-alpha}$ is the ratio between the fundamental amplitude of the saturated waveform and the maximum torque $\gls{sym-tau-max}$, using the describing function derived in \citep{mccabe_force-limited_2024},
%\begin{equation}
%    \alpha = \frac{2}{\pi} \left(\frac{F_p}{\tau_{max}}\sin^{-1}\frac{\tau_{max}}{F_p} +  \sqrt{1 - \left(\frac{\tau_{max}}{F_p}\right)^2} \right).
%\end{equation}

%When $\frac{\gls{sym-F-p}}{\gls{sym-tau-max}}$ = 1 (no saturation), then $\gls{sym-alpha}= 1$;
%when $\frac{\gls{sym-F-p}}{\gls{sym-tau-max}} \rightarrow \infty$ (full saturation), then $\gls{sym-alpha}= \frac{4}{\gls{sym-pi}}$.
