...
\begin{tabular}{lccc}
    ...
    $|\gls{sym-hat-xi-f}| \leq \gls{sym-hat-xi}_{\gls{sym-f},\text{max}}$ &
        ...
        [\gls{sym-mathbf-a}_{(\gls{sym-F-p}\gls{sym-dot-xi})_f\leftarrow VI}]^*
...
\vec{x} = \gls{sym-vec-x-p} - \Gamma \hat{I}_p \vec{n}
    %&= \begin{bmatrix}
    %    1+\Gamma &  0 \\
    %    0 & 1-\Gamma
    %\end{bmatrix}
...
\begin{equation}
\begin{aligned}
    \gls{sym-A-q-mu} &= |\hat{I}_p|^{2},
    \gls{sym-vec-n}^{\,*}\mathbf{Q}_{\mu}\gls{sym-vec-n}& &\in \mathbb{R} \\
    \gls{sym-B-q-mu} &= -\hat{I}_p,
    \left(\gls{sym-vec-x-p}^{\,*}\gls{sym-mathbf-Q-mu} + \vec{a}_{\mu}^{\,*}\right)\gls{sym-vec-n}& &\in \gls{sym-mathbb-C} \\
    \gls{sym-C-q-mu} &=
        \gls{sym-vec-x-p}^{\,*}\gls{sym-mathbf-Q-mu}\gls{sym-vec-x-p}
        +
        2\Re\!\left\{\vec{a}_{\mu}^{\,*}\gls{sym-vec-x-p}\right\}
        -b_{\mu}&
    &\in \gls{sym-mathbb-R}
\end{aligned}
\end{equation}
...
\frac{
    \hat{I}_p^{\,*}\,
    \gls{sym-vec-n}^{\,*}
    \left(\gls{sym-mathbf-Q-mu}\gls{sym-vec-x-p} + \vec{a}_{\mu}\right)
}{
    |\hat{I}_p|^{2}\,
...
\\
S_{\mu} &= \textrm{sign}(\gls{sym-A-q-mu}) = \textrm{sign}\left(\gls{sym-vec-n}^{\,*}\gls{sym-mathbf-Q-mu}\gls{sym-vec-n}\right)
...
\Gamma_{opt} =
\begin{cases}
 0 & \textrm{if } S_{\mu} \gls{sym-C-q-mu} \leq 0 \quad \forall \mu \\[3pt]
 \arg\min\limits_{\gls{sym-Gamma} \in \mathcal{F} \bigcap \gls{sym-mathcal-C}} |\gls{sym-Gamma}|
 & \textrm{otherwise} \\[3pt]
