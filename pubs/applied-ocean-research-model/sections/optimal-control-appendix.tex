...
\Cref{fig:qp-solution-geometry} illustrates the various cases for feasibility and optimality for a toy problem with $\gls{sym-r-mu}=4$, $\gls{sym-S}_{\gls{sym-mu}}=1$, and various $\gls{sym-Gamma-c-mu}$.
\begin{figure}[htbp]
    \centering
...
Toy problems with various constraint circle centers $\gls{sym-Gamma-c-mu}$, all with radius $\gls{sym-r-mu}=4$ and $\gls{sym-S}_{\gls{sym-mu}}=1$, demonstrate the various solution cases in \Cref{eq:gamma-opt-quadratic}.}
