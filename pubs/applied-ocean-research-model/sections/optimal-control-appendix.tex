...
\begin{aligned}
   |\gls{sym-Gamma-cl-mu-star}| &=
    \min_{\gls{sym-mu}} \left\{
        | \gls{sym-f-penalty}\left(\gls{sym-Gamma-cl-mu} (\gls{sym-b-mu})\right) |
    \right\}, \quad
    |\gls{sym-Gamma-int-mu-star-nu-star}| =
    \min_{\gls{sym-mu},\gls{sym-nu}} \left\{
        | \gls{sym-f-penalty}\left(\gls{sym-Gamma-mu-nu}(\gls{sym-b-mu}, \gls{sym-b-nu})\right) |
    \right\}
\end{aligned}
\end{equation}
...
\Cref{sec:appendix-slam} will show that the slamming limits are a function of $\gls{sym-Delta-z-slam}/\gls{sym-H}$, which depends explicitly on the wave height, and $\gls{sym-theta-surf}$, which depends implicitly on both the wave period and wave height through the guessed phase $\angle\gls{sym-xi}$.
