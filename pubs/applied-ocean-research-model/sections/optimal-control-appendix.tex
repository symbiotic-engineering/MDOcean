...
The quadratic matrices $\gls{sym-mathbf-Q-mu}$ in \Cref{tab:qp-constraints} confirm this: $\gls{sym-mathbf-A-hat-q-2}$ and $\gls{sym-mathbf-A-hat-e-2}$ are only positive \emph{semi}definite, indicating constraints that define half-spaces rather than bounded ellipsoids.
...
\Cref{sec:appendix-slam} will show that the slamming limits are a function of $\gls{sym-Delta-z-slam}/\gls{sym-H}$, which depends explicitly on the wave height, and $\gls{sym-theta}$, which depends implicitly on both the wave period and wave height through the guessed phase $\angle\gls{sym-xi}$.
...
\begin{aligned}
    ...
    \left[
        \gls{sym-delta-mu-mu-star-beta}(1-\gls{sym-delta-nu-nu-star-beta})
        \left(
...
\\
    &\left.
        + \gls{sym-delta-mu-mu-star-beta}\gls{sym-delta-nu-nu-star-beta}
        \left(
