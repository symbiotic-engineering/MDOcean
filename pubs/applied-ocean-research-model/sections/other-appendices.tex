...
\end{equation}
which is quadratic in $\hat{\xi}$ and therefore already in a form suitable for the constrained optimal control procedure described in \cref{sec:appendix-qp-solution}.
The remainder of this section develops an explicit expression for the allowable amplitudes under various scenarios.
...
\end{aligned}
\end{equation}
where $\theta$ can take on any value from $\angle\hat{\xi}-kD/2$ to $\angle\hat{\xi}-kD/2$.
\Cref{eq:slamming-min-max} must be evaluated with the value of $\theta$ that creates the minimum value of $\xi_{max,slam}$ and the maximum value of $\xi_{min,slam}$ to ensure lack of slamming across the entire WEC surface.
...
Figure~\ref{fig:slam} visualizes the relationship using the nondimensional parameter $\xi^*=2(\xi_{slam}-\Delta z_{slam,bot})/H$ to map the slamming amplitude onto the range $[-1,1]$.
