...
% Following section~\ref{sec:discussion}, it is interesting to analyze the impact of the more complicated intermediate-wavelength slamming model compared to the simple and conservative short-wavelength model ($\xi_{slam}=\Delta z_{slam}-H/2; \xi^*=-1$).
% Relevant model values for the \hl{minimum-LCOE} float design are plotted for each sea state in \figureautorefname~\ref{fig:slamming-validation}.
% Although the intermediate wavelengths ($\xi^*>-1$) occur for many operational sea states, those sea states are not the ones where the slamming constraint is active (where $\xi_{slam}/\xi-1 \approx 0$) because in the tested wave conditions, large amplitude waves occur only at high periods.
% If the more conservative short-wavelength model is used, the slamming constraint appears violated with a value of $-0.08$ (the small blue portion at the top of the rightmost subplot of \figureautorefname~\ref{fig:slamming-validation} at $T_e=12.5$ s, $H_s=6.75$ m), indicating that the float draft would need to increase by around 8\% to prevent slamming.
...
% Thus, in this case the more complicated, less conservative slamming model has only a minor impact on the analysis, although more substantial impact is expected in seas with co-occurring large-amplitude and low-to-moderate-period waves. 
% \begin{figure}
%     \centering
%     \includegraphics[width=1.1\linewidth]{figs/from-matlab/slamming_model_comparison.pdf}
...
\begin{tabular}{c m{1em} | >{\centering\arraybackslash}m{.5\linewidth} >{\centering\arraybackslash}m{.5\linewidth} }
  && \multicolumn{2}{c}{Drag Coefficent}\\
         && $C_d=0$& $C_d=1$\\ \hline
    \multirow{2}{*}[-3em]{\rot{Hydro Coeffs}} &\rot{Identical}& \includegraphics[width=\linewidth]{figs/manual/wecsim_all_sea_states_a} & \includegraphics[width=\linewidth]{figs/manual/wecsim_all_sea_states_b} \\
     &\rot{Different} & \includegraphics[width=.5\linewidth]{example-image-a} & \includegraphics[width=\linewidth]{figs/manual/wecsim_all_sea_states_d} \\
\end{tabular}
\caption{Dynamic validation showing error in a variety of modeling assumptions}
...
\begin{equation}
    ...
    = \frac{\frac{1}{2} \Re(Z_{damp})|\hat{q}_{damp}|^2} {\frac{1}{2} \Re(Z_{reac})|\hat{q}_{reac}|^2 } 
