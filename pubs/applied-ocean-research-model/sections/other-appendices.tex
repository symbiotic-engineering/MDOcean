...
In regular waves of height $H$, the conditions for slamming can be derived by comparing the vertical position of the bottom of the WEC, $\xi(t)-\Delta z_{slam,bot}$, with the wave elevation $\zeta(y,t)$, which depends on the horizontal position coordinate $y$.
To prevent slamming, the criteria
\begin{equation}\label{eq:slamming-time-domain}
\begin{aligned}
    \xi(t) - \Delta z_{slam,bot} &< \zeta(y,t) \\ 
    |\hat{\xi}|\cos(\omega t+\angle \hat{\xi}) - \Delta z_{slam,bot} &< \frac{H}{2}\cos(\omega t - k y)
\end{aligned}
\end{equation}
...
\begin{equation}
|\hat{\xi}|^2 - H\cos(ky)|\hat{\xi}|\cos\angle\hat{\xi} + H\sin(ky)|\hat{\xi}|\sin\angle\hat{\xi} < \Delta z_{slam,bot}^2-\left(\frac{H}{2}\right)^2
\end{equation}
...
\begin{equation}\label{eq:slamming-squared}
    \left[ |\hat{\xi}|- \frac{H}{2}\cos(\angle\hat{\xi} + ky)\right]^2 < \Delta z_{slam,bot}^2 - (H/2)^2 \sin^2(\angle\hat{\xi} + ky)
\end{equation}
...
\begin{aligned}
    \xi_{max,slam} &= \frac{H}{2} \cos\theta + \sqrt{\Delta z_{slam,bot}^2-\left(\frac{H}{2}\sin\theta\right)^2 } \\
    \xi_{min,slam} &= \frac{H}{2} \cos\theta - \sqrt{\Delta z_{slam,bot}^2-\left(\frac{H}{2}\sin\theta\right)^2 }
\end{aligned}
\end{equation}
...
For the case of $\Delta z_{slam,bot}>H/2$ (wave amplitudes smaller than the draft), \cref{eq:slamming-min-max} reduces to only a maximum amplitude criteria $|\hat{\xi}|<\xi_{max,slam}$ because $\xi_{min,slam}<0$. This aligns with the intuition that a body fixed at the still water line would not exit the water when the wave amplitude is less than the draft.
...
\end{equation}
where the symmetry of \cref{eq:slamming-min-max} has been used to collapse possible values from $[0,2\pi]$ to $[\pi,2\pi]$.

This expression reduces to the simple limits $\Delta z_{slam,bot}-H/2$ and $\Delta z_{slam,bot}+H/2$ for short wavelengths $(\theta=\pi)$ and long wavelengths $(\theta=2\pi)$ respectively.
These limits are also the minimum and maximum values of $\xi_{max,slam}$ for any diameter and wave condition where $\Delta z_{slam,bot}>H/2$.
...
Intuitively, $\theta$ represents how high up the free surface the bottom of the WEC is allowed to get before the edge exits the water, with $\theta=2\pi$ allowing the body to get all the way to the wave crest (top) and $\theta=\pi$ requiring the float to remain fully below the wave trough (bottom).

\subsubsection{Case 2: Large Wave Amplitudes}
In sufficiently large waves ($H/2>\Delta z_{slam,bot}$), \cref{eq:slamming-limit-small-waves} does not apply.
...
\begin{equation}\label{eq:D-slamming}
    kD<2\arcsin\left(\frac{\Delta z_{slam,bot}}{H/2}\right)
\end{equation}
...
\begin{aligned}
    \angle\hat{\xi} \in &\left[0,~~\arcsin\left(\frac{\Delta z_{slam,bot}}{H/2}\right)-\frac{kD}{2}\right] \\
    \cup & \left[2\pi-\arcsin\left(\frac{\Delta z_{slam,bot}}{H/2}\right)+\frac{kD}{2}, ~~2\pi\right]
\end{aligned}
\end{equation}
At the edge-case $\angle\hat{\xi}$ on the inner boundaries of this interval, only a single amplitude is permitted because the values for $\xi_{max,slam}$ and $\xi_{min,slam}$ coincide at $\sqrt{(H/2)^2-\Delta z_{slam,bot}^2}$.
...
The widest permissible amplitude range occurs when the WEC is perfectly in-phase with the incident wave ($\angle\hat{\xi}=0$) and is small with respect to the wavelength ($kD\approx0$), which yields the limits $H/2-\Delta z_{slam,bot} < |\hat{\xi}| < H/2+\Delta z_{slam,bot}$.

\subsubsection{Implementation}
Figure~\ref{fig:slam} visualizes the nondimensional maximum and minimum slamming amplitudes $\xi_{max,slam}/(H/2)$ and $\xi_{min,slam}/(H/2)$ as a function of the worst-case phase angle $\theta$ and the draft to wave amplitude ratio $\Delta z_{slam,bot}/(H/2)$.
This reveals that besides increasing the draft, increasing $\theta$ can prevent slamming, for example by decreasing the diameter or adjusting $\angle \xi$ via control.
If $\Delta z_{slam,bot}<H/2|\sin\theta|$, the design is unviable and the slamming limits are undefined, consistent with the negative right hand side of \cref{eq:slamming-squared} discussed above.
\begin{figure}
    \centering
...
Separately, there exists a symmetrical counterpart to the above slamming condition, where in addition to requiring the bottom of the WEC to remain below the waves, one can require that the top of the WEC remains above the waves, preventing submersion. 
Analogous to \eqref{eq:slamming-time-domain}, we obtain:
\begin{equation}\label{eq:top-slamming}
    \xi(t) + \Delta z_{slam,top} > \zeta(y,t)
\end{equation}
...
Thus, in this case the more complicated, less conservative slamming model has only a minor impact on the \hl{optimization}, although more substantial impact is expected in seas with co-occurring large-amplitude and low-to-moderate-period waves. 
\begin{figure}
    \centering
