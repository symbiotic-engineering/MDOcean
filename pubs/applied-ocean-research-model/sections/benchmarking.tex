...
The popular time-domain hydrodynamic simulation software WEC-Sim \cite{ruehl_wec-simwec-sim_2024} was used to validate the dynamics module.
The WEC-Sim RM3 example was run with regular waves and with the device constrained to oscillate only in heave.
Notably, the RM3 geometry provided in WEC-Sim differs slightly from the published dimensions in the RM3 report, so for validation the dimensions input to MDOcean were adjusted to match WEC-Sim.

WEC-Sim runs utilized hydrodynamic coefficients obtained with the WAMIT BEM for dynamics, and control coefficients calculated with MDOcean for consistency.
MDOcean was run with MEEM as usual, and separately also run with the WAMIT coefficients to distinguish differences caused by disparate hydrodynamic coefficients from those caused by the underlying dynamics. 
...
Results revealed that the drag describing function and MEEM hydrodynamic coefficients have a minor effect assuming a 1-DOF system (9.7\% and 2.7\% error on the average power and maximum amplitude respectively) but a major effect on the 2-DOF system (38.2\% and 28.6\% respectively).
...
Volume and center of buoyancy results are also compared to the WEC-Sim RM3 tutorial geometry, which differs slightly from the nominal RM3 geometry in ways described in \appendixname~\ref{sec:appendix-validation}.
Structural calculations are additionally compared to Reference \cite{previsic_reference_2011}, which provides more detail than the reference model report.
...
The timings in this section were performed on an Ubuntu 20.04 LTS server with a 14-core Intel Core i9-10940X CPU (3.3 GHz base clock) and 256 GB of DDR4 RAM at 3200 MHz, running MATLAB R2024b.

Figure~\ref{fig:runtime-modules} visualizes the breakdown of runtime between modules.
The MEEM hydrodynamics module took the majority (\results[pctRuntimeMEEM]) of the time and is broken down in \figureautorefname~\ref{fig:runtime-hydro}.
The biggest portion was dedicated to evaluating Bessel functions which occur in the semi-analytical solution, another large portion was spent unpacking variables from the cell data structure, and a smaller portion was spent solving the imaginary modes of the dispersion relation and the linear matrix equation.
The simulation was an order of magnitude faster than the Capytaine boundary element method solver for similar convergence levels.
...
This represented a three-order-of-magnitude improvement over the equivalent regular-wave WEC-Sim simulation run in parallel.
Simplifying the dynamics to a single degree of freedom (DOF) achieved another order of magnitude speedup, although the optimization and benchmarking results presented here utilize the 2-DOF model.
...
The structures, geometry, and economics modules were not computationally expensive and together composed the remaining \results[pctRuntimeOther]~of the total runtime. 
