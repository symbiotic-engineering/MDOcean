...
In the circuit representation, equations \eqref{eq:pto-kinematics-motion} and \eqref{eq:pto-kinematics-force} can be represented as three-port transformation matrix, as shown in \figurename~\ref{fig:multiport-circuit-kinematics}.
\begin{figure}
\centering
...
Moving on to the PTO dynamics, a linear model of the drivetrain with mechanical transmission matrix $[a]_m$ takes the form
% impedance
% \begin{equation}
% \begin{bmatrix}
% F_p \\ \tau
...
\end{bmatrix}
\end{equation}
where substituting $[e,q]^T=[F_{p},\dot{\xi}]^T$ yields the floating body mechanical power, $[e,q]^T=[F_{PTO},\dot{X}_{PTO}]^T$ yields the mechanical power at the PTO port, $[e,q]^T=[\tau,\Omega]^T$ yields the generator mechanical power, and $[e,q]^T=[V,I]^T$ yields the generator electrical power. 
...
\subsubsection{Force-Constrained Optimal Reactive Control}
