...
Dots ($\dot{x}=\Re(\hat{\dot{X}}e^{i\omega t})=\Re(i\omega\hat{X}e^{i\omega t})$) indicate time-derivatives.
...
\end{equation}
where the inertial, hydrodynamic radiation, hydrostatic restoring, and drag terms have been combined into a single intrinsic impedance matrix $\hat{\mathbf{Z}}_{i}$ for compactness: $\hat{\mathbf{Z}}_i = -i\omega(\mathbf{M}+\mathbf{A}_h)+(\mathbf{B}_h+\mathbf{B}_d)+\frac{1}{i\omega}(\mathbf{K}_h+\mathbf{K}_d)$.
The PTO kinematics and dynamics are yet to be specified, currently represened by generic impedance matrix $\mathbf{Z}_p = \mathbf{B}_p+\frac{\mathbf{K}_p}{i\omega}$.
\paragraph{PTO Kinematics}
The PTO kinematics describe the relationship between WEC body motions $\vec{\hat{\xi}}$ to PTO motions $\vec{\hat{X}}_{PTO}$, determined by linkages or similar mechanisms and not including subsequent PTO elements such as a drivetrain or generator.
...
Here, we use kinematics to describe the relationship between not only motions but also forces, which implies lossless massless linkages.
Linkage dissipation and inertia could be added to the PTO dynamics in the next section if desired.

\begin{equation}
\vec{\hat{F}}_p = [\mathbf{T_{kin}}(\vec{\hat{\xi}})]^T  \vec{\hat{F}}_{PTO}
\end{equation}

Multibody WECs are frequently underactuated, meaning that the PTO has fewer controlled degrees of freedom than the hydrodynamic bodies.
Underactuated sytems receive considerable attention in robotics \cite{tedrake_underactuated_2024}.

where $\vec{\hat{X}}_{PTO}$ is the vector of PTO displacements and $\mathbf{T_{kin}}$ is the kinematic transformation matrix.

The intrinsic dynamics can be represented as a multiport circuit, as shown in \figurename~\ref{fig:multiport-circuit-intrinsic}.
\begin{figure}
\centering
