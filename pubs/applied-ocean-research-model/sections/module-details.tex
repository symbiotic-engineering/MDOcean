...
For dynamic (time-varying) quantities, the time domain representation is denoted with a lowercase variable $x(t)$, and can also be represented as the real part of a complex signal $x(t)=\Re\left[\tilde{x}(t)\right]$.
...
Before applying the modified frequency domain method described above, we first present the equations of motion for a standard linear frequency domain WEC model.
...
The MDOcean dynamic model assumes regular waves for reasons to be detailed later this section, so the system experiences monochromatic forced oscillation.
...
The complex transfer function from real regular wave height $H$ to the vector of complex body velocity phasors $\vec{\hat{\dot{\xi}}}$ is then: 
\begin{equation}\label{eq:eom-freq-domain}
    \frac{\vec{\hat{\dot{\xi}}}}{H} = \frac{1}{2}\left[\hat{\mathbf{Z}}_i+\hat{\mathbf{Z}}_p\right]^{-1}\vec{\hat{\gamma}}
\end{equation}
where the inertial, hydrodynamic radiation, hydrostatic restoring, and drag terms have been combined into a single intrinsic impedance matrix $\hat{\mathbf{Z}}_{i}$ for compactness:
\begin{equation}\label{eq:intrinsic-impedance}
\hat{\mathbf{Z}}_i = -i\omega(\mathbf{M}+\mathbf{A}_h)+(\mathbf{B}_h+\mathbf{B}_d)+\frac{1}{i\omega}(\mathbf{K}_h+\mathbf{K}_d)
\end{equation}
...
Equations \eqref{eq:eom-freq-domain}-\eqref{eq:intrinsic-impedance} can be visually represented as a multiport circuit, as shown in \figurename~\ref{fig:multiport-circuit-intrinsic}.
\begin{figure}
\centering
    \input{figs/tikz/circuit_intrinsic.tikz}
\caption{Intrinsic dynamics represented as a multiport circuit}
\label{fig:multiport-circuit-intrinsic}
\end{figure}
The excitation forces are represented as dependent voltage sources (diamonds) driven by a single wave elevation source (circle), and the intrinsic and powertrain dynamics are each represented as a two-port impedance matrix.
...
\subsubsection{PTO Kinematics and Dynamics}
We next turn our attention to the PTO kinematics and dynamics, which have so far been left generic in equation~\eqref{eq:eom-freq-domain} as impedance matrix $\hat{\mathbf{Z}}_p$.
...
\vec{\hat{X}}_{PTO} = \mathbf{T_{kin}}(\vec{\hat{\xi}})~ \vec{\hat{\xi}}
\end{equation}
with kinematic transformation matrix $\mathbf{T_{kin}(\vec{\hat{\xi}})}$ that may depend on the body displacements $\vec{\hat{\xi}}$ if the linkages are nonlinear.
...
\vec{\hat{F}}_p = [\mathbf{T_{kin}}(\vec{\hat{\xi}})]^T  \vec{\hat{F}}_{PTO}
\end{equation}
...
\Cref{eq:pto-kinematics-motion,eq:pto-kinematics-force} imply rigid lossless massless linkages, but linkage dissipation, deflection, and inertia could be added to the PTO dynamics if desired.

Multibody WECs are frequently underactuated, meaning that the PTO has fewer controlled degrees of freedom than the hydrodynamic bodies, and $\mathbf{T_{kin}}$ has more columns than rows.
...
In this case, we model two hydrodynamic degrees of freedom (heave of the float and spar respectively) and a single PTO degree of freedom corresponding to the relative heave motion between the float and spar.
The kinematics matrix is therefore the column vector:
\begin{equation}
\mathbf{T_{kin}} = \begin{bmatrix}1 & -1\end{bmatrix}
\end{equation}
...
In the circuit representation, equations \eqref{eq:pto-kinematics-motion} and \eqref{eq:pto-kinematics-force} can be represented as three-port element, as shown in \figurename~\ref{fig:multiport-circuit-kinematics}.
\begin{figure}
\centering
    \input{figs/tikz/circuit_kinematics.tikz}
\caption{Powertrain kinematics represented as a multiport circuit, with the Thevenin equivalent intrinsic dynamics}
\label{fig:multiport-circuit-kinematics}
\end{figure}
Taking the Thevenin equivalent of the intrinsic dynamics and kinematics at the PTO port yields a scalar equivalent circuit with voltage source $\hat{\tilde{F}}_u$ and impedance $\hat{Z}_{i,th}$,  shown in the bottom portion of \figureautorefname~\ref{fig:multiport-circuit-kinematics}.

Moving on to the PTO dynamics, a general linear model of the drivetrain with mechanical transmission matrix $[a]_m$ takes the form
% impedance
% \begin{equation}
% \begin{bmatrix}
% F_p \\ \tau
...
\end{bmatrix}
\end{equation}
where the PTO force $F_{PTO}$ and velocity $\dot{X}_{PTO}$ are found as a function of the generator torque and rotation speed $\tau$ and $\Omega$.
...
The controller can be formulated at either the generator mechanical or electrical port.
The former is more convenient for implementation and constraints, because modern drives typically expect torque commands, while the latter is more convenient for electrical power calculations.
Here we formulate both as linear impedances with damping and stiffness terms, where the mechanical control impedance $\hat{Z}_u$ and electrical load impedance $\hat{Z}_l$ are defined as follows:
\begin{equation}\label{eq:controller-impedances}
\begin{aligned}
    \hat{Z}_l &= \frac{V}{I} = B_l + \frac{K_l}{s}\\
    \hat{Z}_u &= \frac{\tau}{\Omega} = B_u + \frac{K_u}{s}
\end{aligned}
\end{equation}
Combining \cref{eq:controller-impedances,eq:elec-transmission-matrix} gives the following relationship:
...
\end{equation}
Incorporating \cref{eq:mech-transmission-matrix} yields the Thevenin equivalent scalar PTO impedance at the hydrodynamic body port:
...
Transmission matrices $[a]_m$ and $[a]_e$ have so far remained general, able to represent a variety of PTO types. 
Next we specify the exact components assumed in this study: an effective gear ratio $N$, a drivetrain with linear inertia and damping, and a synchronous permanent magnet generator modeled as an ideal gyrator with winding resistance $R_w$ and inductance $L_w$.
...
\caption{PTO (a.) and system (b.) dynamics represented as a multiport circuit}
...
To facilitate this calculation in transmission form, power can be found as the quadratic product of the port variable vector with the two-dimensional exchange matrix $\mathbf{E}_2$,
\begin{equation}
P(t) = e(t)~q(t)
= \frac{1}{2}
\begin{bmatrix}
...
\end{bmatrix}
\end{equation}
where substituting $[e,q]^T=[F_{p},\dot{\xi}]^T$ yields the floating body mechanical power, $[e,q]^T=[F_{PTO},\dot{X}_{PTO}]^T$ yields the mechanical power at the PTO port, $[e,q]^T=[\tau,\Omega]^T$ yields the generator mechanical power, and $[e,q]^T=[V,I]^T$ yields the instantaneous generator electrical power. 
...
\begin{aligned}
    P_{avg} &= \frac{1}{2}\Re(\hat{e} \hat{q}^*) 
    ...
    \begin{bmatrix}
    \hat{e} \\ \hat{q}
    \end{bmatrix}\right)  = \frac{1}{2} \Re(\hat{Z})|\hat{q}|^2 
    \\
    P_{pk} &=  P_{avg} + \frac{1}{2}|\hat{e} \hat{q}^*| = P_{avg}\left( 1 + \sqrt{1 + \left(\frac{\Im(\hat{Z})}{\Re(\hat{Z})}\right) ^2  } \right)
\end{aligned}
\end{equation}
if the port is terminated with complex impedance $\hat{Z}$, where ${}^*$ refers to the conjugate transpose. 
...
If the generator transmission matrix is not used (i.e. $[a]_e = \mathbf{I}$), this efficiency can also be used to approximate losses in the generator itself.
...
Note that this is not necessarily equivalent to an assumption that the generator itself has constant efficiency, since controllers with reactive terms ($K_u\neq 0$) spend some time expending energy, which reverses the direction of the generator efficiency.
...
\subsubsection{Force-Constrained Optimal Reactive Control}\label{sec:optimal-control}
In the absence of constraints, maximizing power at a given port requires impedance matching at that port.
We apply matching at the generator electrical port, so the electrical load impedance $\hat{Z}_l$ must be the complex conjugate of the Thevenin equivalent source impedance $\hat{Z}_{s,th}$ seen from that port.
This is called reactive control, with subscript $reac$:
\begin{equation}\label{eq:matched-load}
    \hat{Z}_{l,reac} = \hat{Z}_{s,th}^*
\end{equation}
Propagating this through to $\hat{Z}_{PTO}$ and $\hat{Z}_u$ via \cref{eq:pto-impedance,eq:control-impedance} respectively gives the optimal control gains $B_{u,reac}$ and $K_{u,reac}$ and optimal Thevenin PTO gains. \hl{fixme}
...
The power-maximizing PTO damping $B_{p,damp}$ is then (\hl{fixme this doesn't apply when you want a port that's not the generator electrical port to be passive})
...
For (1), the expression from \cref{eq:power-avg-peak} for average electrical power can be maximized subject to linear constraints on effort and flow variables can be written as a quadratic optimization problem:
\begin{equation}\label{eq:opt-problem}
\begin{aligned}
& \max_{\vec{x} \in \mathbb{C}^2} & P_{avg,VI} &= \frac{1}{4}\vec{x}^{\,*} \mathbf{E}_2 \vec{x} \\
& \text{s.t.} & \mathbf{A}\vec{x} &\leq \vec{b} \\ 
& & \vec{c}^{\,*}\vec{x} &= d \\
& \text{where} & \vec{x} &= \begin{bmatrix} \hat{V} & \hat{I} \end{bmatrix}^T \\
& & \mathbf{A} &= [\vec{a}_i^{~T} \mathbf{a}_i] \quad \forall i \in [1,\cdots, N] \\
& & \vec{b} &= [\vec{b}_i] \quad \forall i \in [1,\cdots, N] \\
& & \vec{c}^{\,*} &= \begin{bmatrix} 1 & \hat{Z}_s \end{bmatrix} \\
& & d &= \hat{V}_{s,th}
\end{aligned}
\end{equation}
where the linear equality constraint $\vec{c}^{\,*}\vec{x} = d$ enforces the WEC and PTO dynamics through a voltage current relationship at the generator electrical port, and the linear inequality constraints $\mathbf{A}\vec{x} \leq \vec{b}$ can enforce limits on any linear combination of the effort and flow variables at any port.
\hl{Insert explanation of the i's and how you get them from limits at each port, with example of $\tau$ and $\xi$ limits.}

\Cref{eq:opt-problem} has the following solution:
\begin{equation}\label{eq:constrained-qp-solution}
\begin{aligned}
    \vec{x}_{opt} &= 
    \frac{\hat{V}_{s,th}}{2\Re(\hat{Z}_s)}
        \begin{bmatrix} \hat{Z}_s^* (1+\Gamma) \\ 1-\Gamma \end{bmatrix} \\
    P_{avg,VI,opt} &= \frac{|\hat{V}_{s,th}|^2}{8\Re(\hat{Z}_s)}~ (1 - |\Gamma|^2)
\end{aligned}
\end{equation}
where $\Gamma$ is a complex scalar with magnitude between 0 and 1 called the reflection coefficient. The reflection coefficient represents the degree to which the constraints introduce an impedance mismatch. Its value can be calculated analytically via a process described in \cref{sec:appendix-qp-solution}, with $\Gamma=0$ corresponding to the impedance-matched solution (no active inequality constraints).

For (2), actuator limits change the optimal force signal from a pure sinusoid to a saturated waveform, approaching a square wave for sufficiently low force limit.
