...
The float and spar structures therefore experience loads that vary not only in time but also in space along their lengths.
At any given time, the maximum force in space is the greater of the PTO force and the fluid force.
...
\end{aligned}
\end{equation}
In operational seas, $F_p(t)$ is determined by the maximum generator torque $\tau_{max}$ cascaded through the PTO dynamics matrix $\mathbf{a}_{F_p\xi\leftarrow \tau\Omega}$ due to the generator force saturation.
...
\begin{equation}
\max_t F_{p,op}(t) \approx \frac{1}{R} \left(\left| Z_d \hat{\Omega} \right| + \tau_{max} \right)
\end{equation}
while the storm case lacks the control saturation nonlinearity and thus uses the fundamental amplitude $\hat{F}_{p,st}$.
The 

where table~\ref{tab:DLCs} details $F_p$ and $\xi$ based on the assumptions of each design load case.
The vertical design load $F_{heave}$ 
 maximum of the PTO and fluid force amplitudes, which conservatively neglects the spatial distribution of the inertial forces.
...
Solve \eqref{eq:eom-freq-domain} for $F_p$.}\\ 
 Float amplitude, $\xi_f$& \parbox[m]{\linewidth}{\centering Given by \eqref{eq:eom-freq-domain} (2-DOF)}& \parbox[m]{\linewidth}{\centering Given by \eqref{eq:eom-freq-domain} (1-DOF) with $F_p=0$}\\ 
         Spar amplitude, $\xi_s$& Given by \eqref{eq:eom-freq-domain}&Equals $\xi_f$ due to brake\\
 \parbox[m]{\linewidth}{\centering Maximum allowable relative amplitude between float and spar, $|\xi_f-\xi_s|_{max}$ \vspace{8pt}}& \shortstack{$\min(h_{fs,clear},h_{fs,up},$\\$h_{fs,down})$} &N/A\\
 \parbox[m]{\linewidth}{\centering Maximum allowable float amplitude, $|\xi_f|_{max}$ }& $\min(\xi_{f,linear}, \xi_{f,slam})$& N/A \\ %$X_{slam}$\\
 \parbox[m]{\linewidth}{\centering Maximum allowable spar amplitude, $|\xi_s|_{max}$ }& $\min(\xi_{s,linear}, \xi_{s,slam})$& N/A %$X_{slam}$\\
    \end{tabular}
...
In storm seas, the brake force is calculated by 
...
\begin{equation}
    X_{f,linear} = (h-T_{f,2})/10, \qquad X_{s,linear}  = (h-T_s)/10
\end{equation}
