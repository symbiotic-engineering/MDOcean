...
\begin{aligned}
    \gls{sym-I-B}(\gls{sym-r},\gls{sym-theta},\gls{sym-kappa}) &= \int_{-1}^1 \sqrt{1-x^2}\sqrt{1+\gls{sym-r}^2\exp(-2\gls{sym-kappa} x)-2\gls{sym-r}\sin\gls{sym-theta} \exp(-\gls{sym-kappa} x)} dx
    \\
    \gls{sym-I-G}(\gls{sym-r},\gls{sym-theta},\gls{sym-kappa}) &= \int_{-1}^1 \sqrt{1-x^2}\sqrt{1+\gls{sym-r}^2\exp(-2\gls{sym-kappa} x)-2\gls{sym-r}\sin\gls{sym-theta} \exp(-\gls{sym-kappa} x)} \exp(-i\gls{sym-kappa}x) dx
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    \begin{cases}
<<<<<<< HEAD
    \frac{\gls{sym-pi} \gls{sym-mathrm-J-1}((1i-1)\gls{sym-kappa})}{(1i-1)\gls{sym-kappa}} & \textrm{if } \gls{sym-kappa} \neq 0 \\
