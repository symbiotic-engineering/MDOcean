...
\end{equation} and $\mathbf{\eta}_{drag}$ representing the effect of drag on the WEC's ability to capture energy.
...
Rather, it indicates that the drag-free system is so undamped in those sea states that the optimal controller (\cref{eq:matched-load} or \cref{eq:damping-control}) would cause instability, so a sub-optimal controller is used.
...
The dependence is made explicit below:
\begin{equation}
\begin{aligned}
\mathbf{\eta}_{rad} &= \mathbf{\eta}_{rad}(x_{shape}) &
\mathbf{\eta}_{elec} &= \mathbf{\eta}_{elec}(x_{shape},C_{d,f}, C_{d,s},F_{max}, \eta, P_{max}) \\
\mathbf{\eta}_{drag} &= \mathbf{\eta}_{drag}(x_{shape},C_{d,f}, C_{d,s}) &
\mathbf{\eta}_{force} &= \mathbf{\eta}_{force}(x_{shape},C_{d,f}, C_{d,s},F_{max})
\end{aligned}
\end{equation}
