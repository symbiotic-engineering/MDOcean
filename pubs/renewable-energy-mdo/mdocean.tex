...
Parameter sensitivity analysis indicates that LCOE depends more strongly on \sensitiveParamCategoriesMinLCOE~parameters than \insensitiveParamCategoriesMinLCOE~parameters and that the optimal design is sensitive to \sensitiveDesignCategoriesMinLCOE~parameters.
...
Gradient-based optimization for one objective requires \singleObjFcnEvals~function evaluations, while the combination gradient and gradient-free procedure for two objectives requires over \paretoFcnEvals~evaluations.
\end{abstract}
...
\item Optimal designs achieve up to \pctImproveLCOEMinLCOE~LCOE reduction and tend to be \optDesignDescriptionMinLCOE~than the standard RM3 design
\item Sensitivity analysis reveals LCOE is most sensitive to \sensitiveParamCategoriesMinLCOE, while cost is most sensitive to \sensitiveParamCategoriesMinCapex
\item Single- and dual-objective optimization require \singleObjFcnEvals~and over \paretoFcnEvals~function evaluations respectively.
\end{highlights}
...
\input{../../mdocean/simulation/modules/OpenFLASH/pubs/src/meem-appendix.tex}
