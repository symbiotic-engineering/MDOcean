...
For the single-objective formulation, the optimization minimizes the levelized cost of energy ($LCOE$), the metric most commonly used by WEC developers to evaluate market viability \citet{trueworthy_wave_2020} and computed by the economics module \cite{mccabe_development_2026}.
...
\ifdefined\DISSERTATION
    ...
    Early results with nondimensional encodings \citet{mccabe_multidisciplinary_2022} showed the optimizer frequently violating nonlinear constraints and causing simulation failure, so the dimensional encoding is selected.
\else
    ...
    Applied to the draft example, the dimensional encoding renders the relevant priority 1 requirements achievable by linear constraint whereas the nondimensional encoding forces a nonlinear constraint; early results with nondimensional encodings \citet{mccabe_multidisciplinary_2022} showed frequent constraint violation and simulation failure, motivating the dimensional choice.
\fi
...
\ifdefined\DISSERTATION
    ...
    Rather than perform PTO co-design, whose value a number of recent papers (e.g.\ \citet{coe_useful_2023,gaebele_incorporating_2023} and those discussed in \Cref{remdo:sec:lit}) have already proven, this study keeps the PTO design fixed at representative values.
