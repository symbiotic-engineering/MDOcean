...
a maximum LCOE constraint required for the epsilon-constraint procedure that will be described in section~\ref{remdo:sec:multi-obj-process} (\reqref{LCOE}). Note that \reqref{base_static} dominates \reqref{float_spar_draft}, making the latter unnecessary to implement independently.
...
\ref{sec:validation-benchmarking}&100& m\\ 
...
The parameters in \tablename~\ref{remdo:tab:parameters} assume operation in the seas of Humboldt Bay, CA, A-36 structural steel, and nominal RM3 values for the design factor of safety, fixed charge rate, number of devices, and float submergence ratio \cite{RM3}.
