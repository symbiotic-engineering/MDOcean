...
\label{sec:appendix-lit-legend}

The following tables define column-abbreviation legends for the optimization-study comparison in \cref{tab:lit}.
...
\begin{table}
    ...
    \end{tabular}
    \caption{Legend keys for the dynamics and controls columns of \cref{tab:lit}}
...
\begin{table}
    ...
    \end{tabular}
    \caption{Legend keys for other modeling columns of \cref{tab:lit}}
...
\begin{table}
    ...
    \end{tabular}
    \caption{Legend keys for the optimization columns of \cref{tab:lit}}
...
\label{sec:appendix-linear-constraints}

The eight linear inequality constraints of the optimization formulation (\cref{sec:formulation}) are expressed in matrix form below, where each row corresponds to the constraint identifier $g_{L,*}$ given in parentheses:
...
\begin{equation}
    ...
    \end{pmatrix}
\end{equation}
The $-0.01$ values on the right-hand side provide a small numerical buffer against the constraints to avoid model limits. The coefficient for $t_d$ in $g_{L,7}$ is $10^{-3}$ rather than $1$ to convert from millimeters to meters.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Hessian Scaling Procedure}\label{remdo:sec:appendix-scaling}
...
\label{remdo:scaled-standard}
\end{aligned}
\end{equation}
where $\odot$ indicates element-wise matrix multiplication and $\varoslash$ element-wise matrix division.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Hyper-parameters}\label{remdo:sec:appendix-extra-param}
Table~\ref{remdo:tab:hyperparams} documents the hyper-parameters used to control convergence and other numerical aspects of the simulation and optimization.
\begin{table}
    \centering
...
\figureautorefname~\ref{remdo:fig:single-objective-convergence} shows the convergence of the single objective optimization (LCOE minimization) using the nominal RM3 design as a starting point.
\begin{figure}
\centering
...
Figure~\ref{remdo:fig:multistart-parallel-axis} shows the effect of starting point $x_0$ (leftmost column in each subplot) on the optimal design value $x^*$ (middle two columns) and optimal objective value $J^*$ (rightmost two columns).
...
Figure~\ref{remdo:fig:multistart-parallel-axis} also sheds light on the convexity of the search space: local minima are common for all design variables except $T_{f,2}$ and $t_{f,b}$, which usually converge to the same optimal $x^*$ value regardless of the starting point $x_0$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Local Sensitivities}\label{remdo:sec:appendix-sensitivties}
Figure~\ref{remdo:fig:param-sens-local-global-J-star}, \ref{remdo:fig:param-sens-local-x-star}, and \ref{remdo:fig:param-sens-global-x-star} depict the local sensitivities of the optimal objective and optimal design variables, respectively, to parameters. 
\begin{figure}
...
\figureautorefname~\ref{remdo:fig:param-sens-local-global-J-star} compares four ways of computing the objective total derivative local sensitivity, $\frac{dJ^*}{dp}$.
...
The mean absolute error from quadratic to re-optimization-based normalized sensitivities across all parameters is \resultsRE[meanAbsErrorParamSens]\%.
...
Figures~\ref{remdo:fig:param-sens-local-x-star} and \ref{remdo:fig:param-sens-global-x-star} depict the normalized local parameter sensitivities of the optimal design.
...
The global sensitivities of section \ref{remdo:sec:sensitivities} only investigate a few select parameters.
