...
\else
    ...
    OAT does not capture inter-variable coupling \cite{box_statistics_1978}, but the cheap simulation incentivizes proceeding directly to optimization.
\fi
...
\else
    Obtaining gradients for WEC simulations is difficult: existing automatic and analytical differentiation methods cover pseudo-spectral dynamics \cite{bradbury_jax_2018} or BEM and semi-analytical hydrodynamics \cite{rohrer_analytical_2024,khanal_fully_2025,gambarini_gradient_2024} but none supply the body-dimension derivatives of the MEEM hydrodynamics used here, and a symbolic differentiation attempt exhausts memory.
    This study therefore uses finite difference, viable given the cheap simulation. Initial concerns about oscillatory hydrodynamics \cite{mccabe_open-source_2024} proved unfounded, as the overall coefficients are far smoother than their subfunctions.
...
\else
    ...
    The multipliers yield local parameter sensitivities analytically (from $\vec\lambda$, the Hessian, the gradient, and a few finite-difference derivatives), estimating how the re-optimized objective, design, and constraint activity would respond to a parameter change without re-optimizing.
    \Cref{remdo:sec:appendix-sensitivties} gives the details.
...
\else
    While derivative criteria for local Pareto optimality exist \cite{desideri_multiple-gradient_2012}, solvers using them are rare, since Pareto front generation demands exploration rather than the pure exploitation that gradient methods excel at.
    ...
    This retains the benefit of gradient based optimization; guarantee of local optimality; faster convergence.
\fi
