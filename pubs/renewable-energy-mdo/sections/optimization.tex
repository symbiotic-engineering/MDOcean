...
\else
    Obtaining gradients for WEC simulations is difficult: existing automatic and analytical differentiation methods cover pseudo-spectral dynamics \citep{bradbury_jax_2018} or BEM and semi-analytical hydrodynamics \citep{rohrer_analytical_2024,khanal_fully_2025,gambarini_gradient_2024} but none supply the body-dimension derivatives of the MEEM hydrodynamics used here, and a symbolic differentiation attempt exhausts memory.
    This study therefore uses finite difference, viable given the cheap simulation. Initial concerns about oscillatory hydrodynamics \citep{mccabe_open-source_2024} proved unfounded, as the overall coefficients are far smoother than their subfunctions.
...
\else
    While derivative criteria for local Pareto optimality exist \citep{desideri_multiple-gradient_2012}, solvers using them are rare, since Pareto front generation demands exploration rather than the pure exploitation that gradient methods excel at.
