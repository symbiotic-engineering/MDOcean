...
\else
    ...
    or BEM and semi-analytical hydrodynamics \citep{rohrer_analytical_2024,khanal_fully_2025,gambarini_gradient_2024}
    but none supply the body-dimension derivatives of the MEEM hydrodynamics used here,
...
\else
    Local sensitivities exploit the SQP solver's Lagrange multiplier vector $\gls{sym-vec-lambda} = -d\gls{sym-J}/d\gls{sym-vec-g}$ \citep{martins_engineering_2022}, which measures the objective's sensitivity to each constraint; nonzero multipliers mark active constraints, larger values more influential ones.
...
\else
    ...
    This work combines two approaches: the epsilon-constraint method recasts the problem as an ensemble of single-objective problems solved by SQP
