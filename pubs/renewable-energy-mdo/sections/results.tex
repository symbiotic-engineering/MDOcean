...
While reactive control achieves higher power ultimately the lowest LCOE as expected, at lower power levels ($<$\dampingReactivePowerIntersect kW) damping control is actually more cost effective.
...
Each single-objective optimization takes just \singleObjRuntime, converging in \resultsRE[singleObjIters]~iterations and requiring \resultsRE[singleObjFcnEvals]~function evaluations including both objective and gradient evaluation. 
Generating the pareto front takes \paretoRuntime, of which \resultsRE[paretoPctTimeSeeds]~is spent generating seeds via epsilon-constraint and the remaining \resultsRE[paretoPctTimePatternSearch]~on the pattern search. 
