...
Optimal designs ($\vec{x}^*=\text{argmin } J(\vec{x})$) are shown visually in \figureautorefname~\ref{remdo:fig:overlaid-geometry} and numerically in \tablename~\ref{remdo:tab:opt-dv-values}.
\else
Also shown for comparison is the nominal design and a balanced design.
\fi
The latter is a $CAPEX$ minimization run with an additional constraint that the average power must exceed \resultsRE[powerBalanced]~kW, a number chosen based on the shape of the multi-objective pareto front that will be presented in section \ref{remdo:sec:results-multi} with the intent of obtaining a cost reduction at the expense of a small power reduction compared to the minimum LCOE design.
...
\ifdefined\DISSERTATION
The minimum LCOE design is larger in all bulk dimensions, with a particularly large increase in the float diameter, nearly doubling from 20~m to \resultsRE[DfoptMinLCOE]~m.
...
The activity of these bound constraints, which represent priority 2 requirements in the language of section~\ref{remdo:sec:requirements}, indicates that the structures model is at the edge of its validity for describing thin-skinned stiffened shells.
...
The hydrodynamic coefficients and power distributions of each single-objective optimal design, along with the nominal design, are compared in Figures \ref{remdo:fig:overlaid-hydro} and \ref{remdo:fig:overlaid-power-dist} respectively.
The numerical results are shown in \tablename~\ref{remdo:tab:opt-output}.
...
\ifdefined\DISSERTATION
The minimum LCOE design achieves an annual average electrical power of \resultsRE[powerAvgAtMinLCOE]~kW with a structural material mass of \resultsRE[structMassAtMinLCOE]~kg and requires a generator rated for \resultsRE[powerMaxAtMinLCOE]~kW and \resultsRE[forceMaxAtMinLCOE]~kN peak.
This makes its estimated capital cost equal to \resultsRE[capexAtMinLCOE]~\$M, of which \resultsRE[capexDesignAtMinLCOE]~\$M comes from components modeled here as design-dependent (structural material and PTO size), and its estimated LCOE equal to \resultsRE[minLCOE]~\$/kWh under the economic assumptions described in section~\ref{sec:econ}.
This is a \resultsRE[pctImproveLCOEMinLCOE]~LCOE improvement over the nominal design's \resultsRE[nominalLCOE], created through a simultaneous \resultsRE[pctImproveDesignCostMinLCOE]~reduction in design-dependent cost and an \resultsRE[pctImprovePowerMinLCOE]~increase in average power.
...
\ifdefined\DISSERTATION
The CAPEX minimization shows that it is possible to reduce the design-dependent cost by an additional \resultsRE[pctImproveDesignCostMinCapex]~at the cost of a \resultsRE[pctWorsePowerMinCapex]~hit in average power, while the power maximization shows the option of increasing power by an additional \resultsRE[pctImprovePowerMaxPower]~at the expense of a \resultsRE[pctWorseDesignCostMaxPower]~increase in design-dependent cost compared to the minimum LCOE design.
\else
\fi

Finally, \figureautorefname~\ref{remdo:fig:power-matrix-opt} shows a multiplicative breakdown of the power matrix for the minimum LCOE design.
...
\ifdefined\DISSERTATION
Section~\ref{remdo:sec:single-obj-process} introduced the Lagrange multipliers $\vec{\lambda}=-dJ^*/d\vec{g}$ to quantify the activity and influence of constraints at the optimal point.
Values of the nonzero Lagrange multipliers $\vec{\lambda}$ are reported in \figureautorefname~\ref{remdo:fig:lagrange-multiplier}, signifying the increase in objective $J$ for a given increase in active constraint $\vec{g}$.
\else
\fi
...
The utility of the gradient and Hessian are in their ability to calculate other local sensitivity measures, such as the impact of a design variable or parameter that is perturbed from its optimal value.
Local parameter sensitivity analysis occurs in \sectionautorefname~\ref{remdo:sec:sensitivities-local}, and design variable perturbation is examined here.

\figureautorefname~\ref{remdo:fig:delta-J} shows the relative change in objective for a 10\% perturbation of each design variable from its optimal value, $\Delta J/J$, calculated using equation \eqref{remdo:eq:delta-J} and compared against the ``ground truth'' value from simulation re-evaluation.
\else
\fi
...
\ifdefined\DISSERTATION
We first note that the gradient $\vec{\nabla} J$, and therefore the gradient estimate of the objective perturbation $\Delta J$, equals zero for all design variables not involved in an active constraint, as long as the optimization has converged.
Notably, all values are nonzero, indicating that all design variables in some way affect the active constraints identified in \ref{remdo:fig:lagrange-multiplier}.
The values convey the amount that the objective would improve if the design variable changed, although changing this design variable would violate the active constraint(s) that the design variable is involved in.
\else
\fi
...
More accurate estimates of the perturbed LCOE can be obtained by using the Hessian of the objective to account for curvature, but the Hessian would need to be obtained via manual finite differences because the solver only returns the Hessian of the Lagrangian.
\else
\fi
...
\figureautorefname~\ref{remdo:fig:multistart-tree} shows this behavior on a probability tree.
Overall, around one fifth of the optimizations converge to the presumed global minimum.
Therefore, for a 95\% chance of obtaining the global optimum, a multi-start with at least $\textrm{ceil}\left(\frac{\ln(1-0.95)}{\ln(1-0.2)}\right)=\resultsRE[numMultistartRequired]$ linear feasible starting points is required.
...
\ifdefined\DISSERTATION
\figureautorefname~\ref{remdo:fig:multistart-bar} shows a bar chart of the convergence behavior as a function of the starting point for each design variable.
...
More complete multistart results are shown in \appendixautorefname~\ref{remdo:sec:appendix-supplemental-results}.
...
The present section focuses on interpreting the results of the most accurate local sensitivty method: perturbing each parameter a small amount from its nominal value and repeating the optimization, while \appendixautorefname~\ref{remdo:sec:appendix-sensitivties} compares the accuracy and computation time of multiple computation methods.
\else
...
\figureautorefname~\ref{remdo:fig:param-sens-J-star-tornado}, \ref{remdo:fig:param-sens-x-star-tornado}, and \ref{remdo:fig:param-sens-x-star-tornado-2} show Tornado charts for the parameters with highest normalized sensitivities for the objective and design respectively.
...
\ifdefined\DISSERTATION
Analyzing the left side of \figureautorefname~\ref{remdo:fig:param-sens-J-star-tornado}, the value of the minimum LCOE is highly sensitive to the operational wave characteristics, with normalized sensitivities of $-1.4$.
...
Moving to the right side of \figureautorefname~\ref{remdo:fig:param-sens-J-star-tornado}, overall the sensitivities are higher: the minimum LCOE has 5 parameters with normalized sensitivity magnitude above 0.5, whereas the minimum design cost has sixteen such parameters.
...
\ifdefined\DISSERTATION
Meanwhile, \figureautorefname~\ref{remdo:fig:param-sens-x-star-tornado} depicts sensitivity of optimal design variables to parameters, $\frac{d\vec{x}^*}{d\vec{p}}$. 
...
In \figureautorefname~\ref{remdo:fig:param-sens-x-star-tornado-2}, the design variables which minimize design cost are sensitive to these three parameter categories plus economic parameters.
...
The optimal design variables and LCOE corresponding to each of the four locations are shown in Table~\ref{remdo:tab:location}.
\fi
...
The WEC located in Pac-Wave South, OR results in the lowest LCOE, \$\resultsRE[LCOEPacWaveNorth]/kWh, which makes sense because the incident wave energy flux is highest there.
The WEC located in WETS, HI has the highest LCOE, \$\resultsRE[LCOEHawaii]/kWh, aligning with its low incident energy.
The values of the optimal design are similar for the California and Oregon locations, but quite different for Hawaii, with the latter having a \resultsRE[HawaiiDesignCharacteristics].
...
\ifdefined\DISSERTATION
First examining the optimal LCOE design presented in \sectionautorefname~\ref{remdo:sec:results-single}, the lowest force saturation power factor across all sea states in \figureautorefname~\ref{remdo:fig:power-matrix-opt} is $\frac{P_{sat}}{P_{unsat}}=\resultsRE[lowestFmaxFactorMinLCOE]$, with a corresponding force limit of $\frac{F_{max}}{F_{unsat}}=\lowestFsatMinLCOE$~in that sea state. 
\else
\fi
...
Considering all sea states, the optimal force-saturated design sacrifices only \resultsRE[powerLossForceSatMinLCOE]~of average power (around \resultsRE[pctPowerLossForceSatMinLCOE]) and reduces its PTO cost by \resultsRE[pctPTOSavingsForceSatMinLCOE], leading to an LCOE reduction of \resultsRE[pctImproveLCOEForceSatMinLCOE]~compared to the same design with no force saturation, although the unsaturated design violates structural constraints due to the higher force.
If the optimization is instead re-run to ensure constraint satisfaction while not allowing force saturation, the result has a \resultsRE[pctPowerGainForceSatOptMinLCOE]~power gain, \resultsRE[pctPTOCostIncreaseForceSatOptMinLCOE]~higher PTO cost, \resultsRE[pctStructCostIncreaseForceSatOptMinLCOE]~higher structural cost, and \resultsRE[pctLCOEIncreaseForceSatOptMinLCOE]~higher LCOE compared to the optimization with saturation.

\paragraph{Drag}
\ifdefined\DISSERTATION
As section~\ref{remdo:sec:sensitivities-local} alludes to, the low sensitivity to drag coefficient suggests that modeling the drag nonlinearity is perhaps not essential.
...
As described in section~\ref{remdo:sec:multi-obj-process}, the multi-objective analysis gives important insights for cases where the LCOE may not apply or is uncertain due to variation in cost variables modeled as constant, including $OPEX$, $FCR$, and $p_0$.
A pareto front is shown in Figure \ref{remdo:fig:pareto}, with the utopia point marked with a green star, simulated designs marked with squares, and the actual performance of the nominal design as reported in \cite{RM3} marked with a diamond.
...
Figure~\ref{remdo:fig:pareto-lcoe} provides a zoomed-in version with superimposed contours of constant $LCOE$ for reference.
...
There exists a cusp in the pareto front at the point where the average electrical power is around \resultsRE[powerWhereCusp].
\ifdefined\DISSERTATION
As a result of this cusp, the bulk of the pareto front (extending from the least-cost design at \resultsRE[powerAvgAtMinCapex] to around \resultsRE[powerWhereNonconvexEnds]) is non-convex.
...
\ifdefined\DISSERTATION
Figure~\ref{remdo:fig:pareto} inlays geometry visualizations for a few designs highlighted in light blue.
...
Similar trends are explored more quantitatively in Figure \ref{remdo:fig:design-heuristics}.
\ifdefined\DISSERTATION
...
To facilitate comparison against the pareto front in Figure~\ref{remdo:fig:pareto} and visualize clusters, the individual pareto points are shown as black squares on the x-axis of Figure \ref{remdo:fig:design-heuristics}.
\else
...
Using the cost values in the earlier Table~\ref{remdo:tab:opt-dv-values}, unfortunately only \pctCostSavingsFromPTO~of the cost savings come from the smaller PTO, indicating that for a lower power WEC under the previously stated price assumptions, the slightly smaller hydrodynamic design is actually much more important to pursue than the substantially smaller PTO.
\else
...
Meanwhile, the bottom part of \figureautorefname~\ref{remdo:fig:design-heuristics} provides a similar insight on how values change across the pareto front for objectives instead of design variables. % change objectves to outputs - ie LCOE, CF, X_max, all the y* in single objective.
...
\ifdefined\DISSERTATION
Figure~\ref{remdo:fig:constraint-activity} shows which optimization constraints are active along the pareto front.
...
Since this constraint was marked as priority two (required for modeling accuracy given the assumptions), its activity is significant and will be discussed more in section~\ref{remdo:sec:discussion}.
\else
...
\paragraph{Control Scheme}
Figure~\ref{remdo:fig:pareto-lcoe} compares the pareto front for the damping control scheme to the reactive control scheme.
\ifdefined\DISSERTATION
While reactive control achieves higher power ultimately the lowest LCOE as expected, at lower power levels ($<$\resultsRE[dampingReactivePowerIntersect]) damping control is actually more cost effective.
This is because the reactive control scheme requires a larger PTO rating to achieve the same power at low power levels, as well as larger structures to withstand the higher forces.
Incorporating the unmodeled effect that PTOs with both motoring and generation capability can be more expensive than those with only generation capability would further increase the appeal of damping control at low power levels.
This trend also suggests the possibility that the optimal controller for a given power level may be an alternative controller (neither damping nor reactive control).
\else
In other words, while reactive control yields the highest power for a given design, it is not necessarily the highest power for a given cost, considering that PTO and structural cost scale with the controller.
\fi
...
\ifdefined\DISSERTATION
Figure~\ref{remdo:fig:optim-runtimes} shows the runtime for each optimization and analysis routine, as run in parallel on the 14-core server described in section~\ref{sec:sim-runtime}. 
...
Each single-objective optimization takes just \resultsRE[singleObjRuntime], converging in \resultsRE[singleObjIters]~iterations and requiring \resultsRE[singleObjFcnEvals]~function evaluations including both objective and gradient evaluation. 
Generating the pareto front takes \resultsRE[paretoRuntime], of which \resultsRE[paretoPctTimeSeeds]~is spent generating seeds via epsilon-constraint and the remaining \resultsRE[paretoPctTimePatternSearch]~on the pattern search. 
Gradient-based post-optimality sensitivities run rapidly in just \resultsRE[gradientSensRuntime], while sensitivities obtained via re-optimization take \resultsRE[reoptimSensRuntime].
