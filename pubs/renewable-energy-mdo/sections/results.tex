...
\else
    The balanced design is a $CAPEX$ minimization constrained to exceed \resultsRE[powerBalanced]~kW average power (chosen from the Pareto front of \cref{remdo:sec:results-multi}), trading a small power reduction for lower cost, preferable when design-dependent capital cost matters more than modeled here.
\fi
...
\else
    \Cref{remdo:fig:power-matrix-opt} shows a multiplicative breakdown of the minimum-LCOE power matrix into maximum capture width, radiation efficiency, drag efficiency, the $F_{max}$ force-limit factor (departing from unity only near resonance at high wave heights), and electrical efficiency; their product with the annual hours per sea state yields annual energy production.
\fi
...
\ifdefined\DISSERTATION
    ...
    The nonzero multipliers are reported in \cref{remdo:fig:lagrange-multiplier}, each signifying the increase in objective $J$ per unit increase in an active constraint $g$.
\else
    The Lagrange multipliers $\vec{\lambda}=-dJ^*/d\vec{g}$ (\cref{remdo:sec:single-obj-process}), reported in \cref{remdo:fig:lagrange-multiplier}, quantify each active constraint's influence on the objective.
\fi
...
\else
    \Cref{remdo:fig:multistart-bar} shows convergence behavior versus each design variable's starting value: the float draft $T_{f,2}$ and bottom plate thickness $t_{f,b}$ converge to the same optimum regardless of start (nearly unimodal), while the others are highly multimodal.
...
\ifdefined\DISSERTATION
    ...
    The minimum design cost is most sensitive ($+2.1$) to the nondimensional damping plate stiffener height $h_{stiff,d}/h_{1,stiff,d}$,
    suggesting it be added as a design variable.
    Other high sensitivities such as
    $D_d/D_s$ ($+1.7$),
    $D_{d,tu}$ ($-1.6$),
    $D_{d,min}$ ($-1.1$),
    $w_{stiff,d}/h_{1,stiff,d}$ ($-1.0$),
    ...
    Site parameters ($h$, $-1.8$; $H_s$, $-1.6$; $T_{struct}$, $+1.4$; $H_{s,struct}$, $+1.1$) are also influential;
    notably the sign of $T_{struct}$ flips relative to the minimum-LCOE point, so a larger structural period decreases minimum LCOE but increases minimum design cost.
...
\else
    For the minimum LCOE (left of \cref{fig:param-sens-J-star-tornado}), the dominant sensitivities are
    ...
    Two counterintuitive findings stand out: the sign of the structural-period sensitivity $T_{struct}$ flips between the two objectives,
...
\ifdefined\DISSERTATION
    ...
    $D_d/D_s$ strongly affects four design variables ($D_s$, $t_{sr}$, $h_s$, $h_{stiff,f}$), indicating $D_d$ should be a design variable;
    $T_{f,1}/T_{f,2}$ strongly affects the overall draft $T_{f,2}$, indicating $T_{f,1}$ should be one too;
    $T_s/D_s$ affects $h_s$ and $h_{fs,clear}$, indicating $T_s$ should be added alongside $h_s$;
    and $D_{f,in}/D_s$ strongly affects $h_{stiff,f}$, suggesting $D_{f,in}$,
...
%For these highly sensitive parameters, it is worthwhile to perform a global sensitivity analysis to observe any nonlinearity. Figure~\ref{remdo:fig:param-sens-global} and \ref{remdo:fig:x-opt-sens-global} demonstrate the new optimal objective and optimal design variable values respectively under the re-optimization-based OAT global parameter sensitivity process described in section~\ref{remdo:sec:single-obj-process}.
...
\else
    ...
    This captures interactions among wave height, period, depth, and storm state that the OAT analysis cannot, and tests whether the optimal design is robust to location which is relevant because global deployability is a core viability requirement \cite{bull_systems_2017}. \Cref{remdo:tab:location} gives the results.
\fi

\begin{table}\setlength\arraycolsep{1pt}
...
\ifdefined\DISSERTATION
    ...
    $P_{sat}/P_{unsat}=\resultsRE[lowestFmaxFactorMinLCOE]$, at a force limit of $F_{max}/F_{unsat}=\lowestFsatMinLCOE$ in that sea state.
...
\else
    ...
    which is surprising given the active slamming constraint, though a global re-optimization at $C_d=0$ is needed to confirm.
\fi
...
\else
    The multi-objective optimization simultaneously trades off design-dependent capital cost $C_{design}$ against average electrical power $P_{avg,elec}$ (\cref{remdo:sec:multi-obj-process}),
...
%An additional point of note on the Pareto front of Figure \ref{remdo:fig:pareto} is the LCOE and coefficient of variation of a typical solar array plotted for comparison (data from \cite{borettiSolar}). Even the cheapest WEC design is still almost an order of magnitude more expensive than solar, indicative of the immaturity of WEC technology and the need for further innovation and cost reduction. On the bright side, many of the designs achieve a lower coefficient of power variation than solar, indicating the potential for wave energy as a relatively steady grid-connected renewable. Finally, it is worth highlighting that the amount of power variation directly affects the required energy storage, so the local cost of storage affects the position along the Pareto front that is optimal for a given application.
...
\figurename~\ref{remdo:fig:pareto-epsilon-runtime-sweep} shows the effect of the number of epsilon-constraint seeds on the number of function evaluations and hypervolume. (\hl{Analysis is left for during scrutineering.})
