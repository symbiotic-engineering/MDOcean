...
Optimization outcomes and targeted sensitivity analyses for the formulation and model assumptions show ( ---- ).
...
Most violations are due to fatigue and ultimate failure of the damping plate, 
and a few designs violate hydrostatic stability ($g_{NL,5}$) or damping plate geometric compatibility ($g_{L,7}$).
Of particular note, increasing the float diameter obtains the most significant LCOE decrease.  The LCOE attains a local minimum at a float diameter around 1.5 times the nominal value, but the plate fails structurally there.
The PTO power and force ratings also attain local minima due to the tradeoff of power generation and cost, though the structural variables do not since they exist solely to satisfy constraints.
...
% it's looking good (large decrease especially with D_f, and local optim behavior), but we need to be careful about infeasibility because a lot of constraints are violated


\begin{figure}[htbp]
\centering
\includegraphics[\FigureOptions{re:figs/from-matlab/experiments_ratios.pdf}]{figs/from-matlab/experiments_ratios.pdf}
\caption{Exploratory OAT sweep of design variables to understand tradeoffs prior to optimization. Feasible and infeasible designs are indicated with an O and X respectively.}
...
%This suggests formulation changes for future work: incorporate manufacturing-driven max dimension constraints, different structures model, and allow Tf2/hf to be a design variable.
...
\else
    \Cref{remdo:fig:constraint-activity} shows the active constraints along the front: one structural variable at its lower bound, the spar natural frequency constraint (a priority 2 modeling-accuracy constraint whose activity is discussed in \Cref{remdo:sec:discussion}), and the structural factor-of-safety, force, and power limits.
\fi
\begin{figure*}[htbp]
    \centering
...
\else
    ...
    This suggests the cost-optimal controller at a given power may be neither pure scheme, as reactive control maximizes power for a given design but not for a given cost. This suggests that besides $F_{max}$ and $P_{max}$, $P_{reactive,max}$ should be a design variable and a dynamic constraint.
\fi
