...
Meanwhile, the larger draft $\gls{sym-T-f-2}$ increases the upper limit on oscillation amplitude imposed by the slamming constraints $\gls{sym-g-NL-23}$-$\gls{sym-g-NL-239}$, which brings actual power generation closer to the maximum.
...
\ifdefined\DISSERTATION
    ...
    so for a lower-power WEC under these price assumptions the slightly smaller hydrodynamic design matters far more than the substantially smaller PTO.
...
\ifdefined\DISSERTATION
    ...
    Although the cost model is uncertain, it still represents a significant improvement over typical WEC cost proxies such as wetted surface area and hull volume.
...
We term this the ``base cost of survival'' because it is the minimum cost of a two-body point absorber WEC that survives the storm DLC and satisfies all other constraints, even if it produces negligible power.
This quantity should interest designers of WECs for PBE applications:
it can be compared to the maximum cost a customer is willing to pay for an extremely low-power WEC,
and to the infinitesimal-power fixed cost of other energy sources such as solar, wind, and other WEC archetypes, to determine if a two-body point absorber WEC is a sensible option for such applications.
If the WEC is infeasible, the gap determines the reduction to the base cost of survival that is required for viability.
...
(or more generally enhancing the economic viability of WECs with power requirements under 100~kW,
...
While fatigue-minimizing WEC control has been explored before \cite{}, this study is the first to quantify its potential economic impact and relevance for low-power WECs.
...
\ifdefined\DISSERTATION
    ...
    This is important because global deployability is a core WEC viability requirement \citep{bull_systems_2017} and a single design usable across wave conditions facilitates economies of scale.
...
\ifdefined\DISSERTATION
    A survey indicates that 40\% of WEC developers design for site-agnostic deployment, so global deployability is of industrial interest \citep{trueworthy_wave_2020}.
    ...
    This departs from \citet{de_andres_adaptability_2015}, who find that small (4~m diameter) WECs optimize their LCOE proxy regardless of dominant period.
    This demonstrates that a more thorough multidisciplinary model is essential to draw accurate design conclusions.
\else
    A survey finds 40\% of WEC developers design for site-agnostic deployment \citep{trueworthy_wave_2020}.
...
\ifdefined\DISSERTATION
    ...
    We observe a reduction in drag efficiency near the resonant period, where the oscillation amplitude and thus the relative WEC-water velocity is particularly high.
...
Paradoxically, these features can be interpreted to both confirm and contradict the argument that WECs should prioritize consistent capture of small frequent waves.
In terms of the PTO sizing, the WEC should indeed prioritize the more common low-energy sea states and sacrifice operation in the largest waves.
Of the remaining waves, however, the WEC's hydrodynamic design should split its priority between the small-yet-common and large-yet-rare sea states, while de-emphasizing the waves that are moderate in both size and probability.
This can be traced directly back to the nature of the design space.
The PTO sizing design variables specify a \textit{maximal cut-off point} in some aspect of the WEC's dynamic response (either force or power).
...
but also a \textit{nominal operating point} in the WEC's dynamic response (e.g. draft sets its resonant frequency while diameter sets the damping ratio and bandwidth).
Conceptually, setting a larger maximum cut-off point does nothing to penalize the WEC's energy generation in smaller waves, so the only tradeoff is between cost and power.
...
This feat is likely made easier by the ability to independently shape the float and spar impedances to obtain a high-order relative response, which is not possible in a single-body WEC. 
...
A previous study compares structural mass per unit hull volume for marine structures including WECs and ship hulls, finding $278.76$~kg/m\textsuperscript{3} for steel structures \citep{roberts_bringing_2021}, with a 95\% confidence interval of $\pm280$ metric tons.
\ifdefined\DISSERTATION
...
\ifdefined\DISSERTATION
    ...
    for instance, maximum dimensions may be limited by roadway transport, as for wind turbines, constraining component dimensions to 4.3--4.6~m \citep{cotrell_analysis_2014}, though relating maximum transported component size to maximum installed WEC dimensions requires manufacturability and on-site assembly analysis beyond this scope.
    For guidance on requirements and constraints in WEC design optimization, the reader is referred to \citet{bull_systems_2017,babarit_stakeholder_2017} for a comprehensive breakdown and mapping of WEC requirements, and to \citet{trueworthy_wave_2020} for how developers prioritize requirements in practice.
\else
    ...
    \citet{bull_systems_2017,babarit_stakeholder_2017,trueworthy_wave_2020} give comprehensive WEC requirement guidance.
\fi
...
\else
    All prior large-scale ($\geq 8$ design variables) WEC optimizations use the genetic algorithm (\Cref{remdo:sec:lit}), requiring roughly 2{,}000--12{,}000 function evaluations to converge \citep{khanal_multi-objective_2024,garcia-teruel_reliability-based_2021,cotten_multi-objective_2022,abdulkadir_control_2024}.
