...
The optimized design is larger in all bulk dimensions, most strikingly in float diameter: $D_f$ \hl{nearly doubles} from 20~m to \resultsRE[DfoptMinLCOE].
...
\ifdefined\DISSERTATION
    Six constraints are active at the optimum: one structural thickness lower bound, three structural factors of safety, and \hl{two} amplitude constraints.
    \hl{While the storm (maximum) DLC limits the float and damping plate, it is the operational (fatigue) DLC that drives the column sizing.}
    The amplitude constraints and the damping plate factor of safety in the \hl{storm} DLC have the highest multipliers,
...
\ifdefined\DISSERTATION
    Examining the direction of change, with increasing cost and power (left to right) most design variables either \hl{increase monotonically} ($P_{\text{max}}$, $D_s$, $t_{s,r}$) or \hl{nearly so with one brief decline} ($F_{\text{max}}$, $D_f$, $h_{1,\text{stiff},d}$, $h_s$).
    \hl{Only two mainly decrease}: $T_{f,2}$ and $h_{fs,\text{clear}}$, both involved in the float amplitude constraints.
    Of the \hl{remaining three}, $t_d$ and $t_{f,b}$ (structural thicknesses at their lower bound) stay essentially constant apart from small upward excursions in $t_d$ \hl{near 2\%, 23\%, and 60\% along}, while $h_{\text{stiff},f}$ shows \hl{several small oscillations}.
...
\ifdefined\DISSERTATION
    ...
    Power output is \hl{largely linear}, while design cost shows a \hl{discrete slope change near 5\%} and a gradual one beyond.
    The discrete change corresponds to a \hl{shift in PTO sizing strategy}: from 0-5\% along, the cost-power trade is made by adjusting maximum power, whereas from 5-70\% it is made by adjusting maximum force at constant maximum power.
...
\fi
\hl{Takeaway: we need to do further studies to say for sure and better understand exactly why this model agrees in some places and disagrees in others.}
...
\ifdefined\DISSERTATION
    ...
    with a normalized sensitivity of \hl{-1.4}.
...
\ifdefined\DISSERTATION
    ...
    \hl{Examining geometric parameters for promotion to design variables:}
