...
\ifdefined\DISSERTATION
    ...
    of which \resultsRE[capexDesignAtMinLCOE] is design-dependent (structural material and PTO size),
    and its estimated LCOE is \resultsRE[minLCOE] under the economic assumptions of \citet{mccabe_development_2026}.
...
\ifdefined\DISSERTATION
    ...
    The amplitude constraints and the damping plate factor of safety in the operational DLC have the highest multipliers,
...
\ifdefined\DISSERTATION
    ...
    As described in \Cref{remdo:sec:multi-obj-process}, this gives insight for cases where LCOE may not apply or is uncertain due to cost variables modeled as constant, including $OPEX$, $FCR$, and $\gls{sym-p-0}$.
    ...
    All Pareto-optimal designs cost significantly less than the nominal RM3, spanning powers both below and above it.
    %\Cref{remdo:fig:pareto-lcoe} provides a zoomed view 
    The minimum-LCOE design lies at the tangency between the Pareto front and the contours and very nearly coincides with the maximum-power design.
...
\else
    ...
    giving insight where LCOE is inapplicable or uncertain in its constant-modeled cost terms ($OPEX$, $FCR$, $\gls{sym-p-0}$).
    The Pareto front (\Cref{remdo:fig:pareto-lcoe}) shows all designs costing significantly less than the nominal RM3 \citep{RM3} across a range of powers;
...
\ifdefined\DISSERTATION
    ...
    This raises the question of how much of the cost savings at lower power come from smaller PTO versus lower material cost:
    if PTO cost dominates, companies selling devices at multiple power levels could keep one geometric and structural design and vary only the PTO,
    easing design effort and economies of scale.
    Using the costs in \Cref{remdo:tab:opt-dv-values}, however,
    only \resultsRE[pctCostSavingsFromPTO]~of the savings come from the smaller PTO,
    so for a lower-power \gls{acr-wec} under these price assumptions the slightly smaller hydrodynamic design matters far more than the substantially smaller PTO.
    ...
    which wrongly examined only the change in optimal design without weighting the relative cost impact of PTO and dimensions.
\else
...
\ifdefined\DISSERTATION
    ...
    We also observe a shift in the PTO sizing strategy along the front: from 0-10\% and 18-25\% along, the cost-power trade is made by adjusting maximum power, whereas in the remaining regions it is made by adjusting maximum force at constant or decreasing maximum power.
...
This asymmetry is a consequence of the fixed cost in the numerator of LCOE and the absence of any fixed additive power in the denominator.
...
\ifdefined\DISSERTATION
    ...
    While the multi-objective optimization presented here addresses LCOE's high uncertainty better than most other approaches, limitations remain.
...
We term this the ``base cost of survival'' because it is the minimum cost of a two-body point absorber \gls{acr-wec} that survives the storm DLC and satisfies all other constraints, even if it produces negligible power.
This quantity should interest designers of \glspl{acr-wec} for PBE applications:
...
This implies that the base cost of survival does not depend entirely on the storm DLC as might be expected, since the model assumes that the PTO locks and the control scheme is irrelevant in a storm.
...
the way PTO force, power, and amplitude are currently handled.
...
\ifdefined\DISSERTATION
    ...
    It shows not only which locations are most favorable through the resulting LCOE, but also whether the optimal design is robust to location through the resulting design variables.
    ...
    Wave resource data for four U.S.\ locations (Humboldt Bay, CA; Pac-Wave North and South, OR; and the Wave Energy Test Site, HI) is obtained from the NREL SAM repository \citep{janzou_sam_2022}, sourced from a 2015 Sandia report \citep{dallman_characterization_2015}.
...
\else
    ...
    This captures interactions among wave height, period, depth, and storm state that the OAT analysis cannot, and tests whether the optimal design is robust to location, which is relevant because global deployability is a core viability requirement \citep{bull_systems_2017}.
...
\ifdefined\DISSERTATION
    ...
    Meanwhile, the location sensitivity (\Cref{remdo:tab:location}) shows a significantly larger optimal float in Hawaii, the site with the lowest common wave period.
    This departs from \citet{de_andres_adaptability_2015}, who find that small (4~m diameter) \glspl{acr-wec} optimize their LCOE proxy regardless of dominant period.
...
\else
    ...
    MDOcean's results agree that power maximization does not align with LCOE minimization, but show the cost penalty as PTO rather than structural cost, and show a significantly larger optimal float in Hawaii (\Cref{remdo:tab:opt-dv-values,remdo:tab:opt-output,remdo:tab:location}), which contradicts \citet{de_andres_adaptability_2015} and demonstrates the value of a fuller multidisciplinary model.
\fi
...
% 195 - 201
\ifdefined\DISSERTATION
    Because sensitivities show that the optimal LCOE is particularly sensitive to efficiencies and site characteristcs that drive energy generation,
    \Cref{remdo:fig:power-matrix-opt} shows a multiplicative breakdown of the power matrix for the minimum-LCOE design to reveal which factors most limit energy production.
...
In terms of the PTO sizing, the \gls{acr-wec} should indeed prioritize the more common low-energy sea states and sacrifice operation in the largest waves.
...
The PTO sizing design variables specify a \textit{maximal cut-off point} in some aspect of the \gls{acr-wec}'s dynamic response (either force or power).
...
Thus, the importance of balancing cost, power, and probability at the grid scale becomes evident through both parameter sensitivities and the power-matrix breakdown and explains the mixed results between the PTO and hull design variables.
...
\ifdefined\DISSERTATION
    ...
    Transformative improvements include creative concepts that avoid these constraints altogether, storm-survival strategies such as submerging the \gls{acr-wec}, changing shape with control surfaces, or altering the PTO control strategy to reduce force and amplitude, whose viability can be readily estimated by weighing the additional costs (e.g.\ actuator and control hardware and software) against the performance gains, again using the Lagrange multipliers of \Cref{remdo:fig:lagrange-multiplier}.
\else
...
\ifdefined\DISSERTATION
    ...
    This is relevant for small-scale Powering the Blue Economy applications with a fixed power requirement, where it is unclear whether multiple smaller \glspl{acr-wec} or one larger \gls{acr-wec} is preferable.
    ...
    For example, a high sensitivity of the optimal structural design variables to the powertrain design variables $\left(\frac{dx^*_{\text{struct}}}{dx_{pto}}\right)$ would support this study's choice of fully coupled MDO \citep{mccabe_development_2026}, while a low value would support disciplinary decoupling.
...
\else
    ...
    Adding $N_{WEC}$ matters little for LCOE minimization but is valuable in a modified multi-objective analysis using array power, relevant to fixed-requirement Powering the Blue Economy applications weighing many small \glspl{acr-wec} against one large one.
...
\newcommand{\achievableDesignStudiesRE}{
    ...
    4) Add $N_{WEC}$ as a design variable and see when it's preferable to do many smaller \glspl{acr-wec} vs one bigger WEC
...
Would require not only MEEM for the different \gls{acr-wec} types, but also semi-analytical structural models for each one, or the decision to integrate a lightweight FEA with plate/shell element capabilities (ie pynite).

	4) make lifetime a design variable, and adjust storm-wave size accordingly. This could address what OPEX cost threshold would justify intentionally designing ``single-use'' \glspl{acr-wec}, which might be considered in PBE applications.
Would require integration with WDRT or similar to get the environmental contour for a given lifetime.
	...
	6) generator physics and model - building off the PTO CCD study that is easily realizable with the current model, a generator model would unlock \gls{acr-ccd} with the generator itself, for example whether a high-torque (expensive) generator is worth it compared to a low torque generator.
    ...
    A simplified generator model is explored in the RM3 CCD study \citep{anderson_re-imagining_2024}, or a full generator model as in the offshore wind MDO study \citep{barter_beyond_2023}.
...
\else
    ...
    The most transformative additions, such as irregular waves (enabling realistic fatigue and power-variability studies), additional \gls{acr-wec} archetypes (addressing design convergence), multi-region MEEM hydrodynamics, and a generator physics model (enabling generator \gls{acr-ccd}, as in \citet{anderson_re-imagining_2024,barter_beyond_2023}), would unlock qualitatively new design studies.
