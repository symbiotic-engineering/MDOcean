...
\ifdefined\DISSERTATION
    ...
    This differs from preliminary work \citep{mccabe_multidisciplinary_2022}, which wrongly examined only the change in optimal design without weighting the relative cost impact of PTO and dimensions.
\else
...
If you want to cut the design cost in half, you are cutting the power into three, which is a big power penalty. Power falls off steeply.
Diminishing returns.

\subsubsection{Minimum Cost and Maximum Power Designs}

I want to highlight these points because the one on the right (max power) is the highest power you can get.
...
\fi
Looking at this tradeoff helps arbitrate the dispute about whether you should assume reactive control (which is what people doing CCD more recently will tend to advocate) or damping control (which is what historically was the assumption for hardware simplicity).
...
\fi
Takeaway: we need to do further studies to say for sure and better understand exactly why this model agrees in some places and disagrees in others.
