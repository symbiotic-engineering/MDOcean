...
for matl = 1%1:2:3 %b.M_min : b.M_max
    ...
    for i=1:2
        ...
        if solver_based
            ...
            % First run unscaled optimization to determine scale factor
            [X_opt_un,~,~,~,~,~,hess_un] = fmincon(problem)
            Diag=diag(hess_un)
            cond(hess_un)
            scale = 1./sqrt(Diag);
            
            % Then reformulate scaled optimization problem     
            problem_s = problem;
            problem_s.objective = @(x) problem.objective(x .* scale);  
            problem_s.nonlcon   = @(x) problem.nonlcon(x .* scale);
            %problem_s.objective = @(x) generatedObjectiveLCOE(x.*scale, {p});
            %num_constraints = numel(fieldnames(prob.Constraints));
            %problem_s.nonlcon=@(x)generatedConstraints(x.*scale, repmat({p},1,num_constraints));
            X_sx0 = 1./(scale);
            problem_s.x0 = X_sx0 .* problem.x0;%X_opt_un;
