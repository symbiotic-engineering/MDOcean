...
def remove_autogenerated_docstring(content):
    """Remove existing autogenerated docstring from content.
    
    Removes docstring lines after function, keeps marker before function.
    """
    lines = content.split('\n')
    result = []
    in_autogen_doc = False
    
    for i, line in enumerate(lines):
        stripped = line.lstrip()
        
        # Keep marker lines (they're before function)
        if '%[AUTOGENERATED]' in stripped:
            result.append(line)
            continue
        
        # Mark when we're in autogenerated docstring content (after function)
        if in_autogen_doc:
            if stripped and not stripped.startswith('%'):
                # Hit non-comment code, docstring is over
                in_autogen_doc = False
                result.append(line)
            elif stripped.startswith('% :param') or stripped.startswith('% :returns') or stripped.startswith('% Function') or stripped == '%':
                # Skip docstring lines
                continue
            else:
                result.append(line)
        else:
            # Check if this is a function line (start of autogenerated docstring block)
            if stripped.startswith('function '):
                result.append(line)
                in_autogen_doc = True  # Docstring comes right after function
            else:
                result.append(line)
