...
This suggests that it is primarily the initial value of the draft $T_{f,2}$, rather than the float thickness $t_{f,b}$, that is important for arriving at the global optimum.
Figure~\ref{fig:multistart-parallel-axis} also sheds light on the convexity of the search space: local minima in \hl{(DV)} are common, while \hl{(other DVs)} appear to have a single optimal value regardless of the starting point.
...
\hl{This requires further investigation to explain.}
