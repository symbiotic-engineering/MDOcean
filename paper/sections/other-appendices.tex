...
In regular waves of height $H$, the conditions for slamming can be derived by comparing the vertical position of the bottom of the float, $x(t)-T_f$, with the wave elevation $\zeta(y,t)$, which depends on the horizontal position coordinate $y$.
...
\end{aligned}
\end{equation}
must be true at all times and over all positions where the float could exit the waves, $-D_f/2<y<D_f/2$.
...
\end{aligned}
\end{equation}
This expression reduces to the simple limits $T_f-H/2$ and $T_f+H/2$ for short wavelengths $(\theta=0)$ and long wavelengths $(\theta=\pi)$ respectively.
These limits are also the minimum and maximum nonzero critical slamming amplitudes for any design and wave condition.
For intermediate wavelengths, the variable $\theta$ accurately accounts for the effect of body diameter and the phase offset of float motion from the waves.
Intuitively, $\theta$ represents how high up the free surface the bottom of the WEC is allowed to get before the edge exits the water, with $\theta=\pi$ allowing the float to get all the way to the wave crest and $\theta=0$ requiring the float to remain fully below the trough.
...
If $T_f<H/2\sin\theta$, slamming occurs even for a stationary body, and the critical slamming amplitude is set to zero to indicate an unviable design.
\begin{figure}
    \centering
...
Following section~\ref{sec:discussion}, it is interesting to analyze the impact of this more complicated slamming model compared to the simple and conservative short-wavelength model ($X_{slam}=T_f-H/2; X^*=-1$).
...
This gives $x_{min}\approx2.2$~m for the nominal float design.
...
\end{equation}
using a similar inscribed circle scheme.
...
Unlike the uniform pressure on the bottom plate, the top plate is subject to a load distributed over a small circle of radius $r_0$, where $r_0$ is assumed to be at least half the thickness.
...
The other float plates besides the top and bottom ones experience only the edge reaction moments from the top and bottom plates.
\hl{Explain the implication of this and that MDOcean just scales.}
...
The critical buckling stress $\sigma_{buckle}$ is found according to \cite{noauthor_requirements_2022}.
...
\figureautorefname~\ref{fig:param-sens-local-J} compares four ways of computing the objective total derivative local sensitivity, $\frac{dJ^*}{dp}$.
...
While the first three methods take \hl{XX} minutes to run, the fourth method takes \hl{XX} minutes, since \hl{XX} separate optimizations are performed.
...
Future work could obtain fuller global sensitivities through re-optimization or a variety of other methods such as Sobol variance decomposition or Method of Morris \cite{reed_uncertainty_2022}.
