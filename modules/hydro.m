function [F_heave,F_surge, m, D_env] = hydro(t, s, m_float, D_sft, D_i,D_or,t_r,rho_w, g, Hs, T, t_f)
w = 2*pi/T;        % frequency
k = w^2 / g;        % wave number
...
vol_rp = A_wrp*t_r;%vertical column submerged volume

% hydrodynamic coefficients
A       = 1/2 * rho_w * 4/3 * pi * r_sf^3 * 0.63 * ones(1,size(s,2)); % added mass
...
F_surge_rp = 2 * (Hs/2) * rho_w * g * vol_rp * (1 - exp(-k*(t_r)));%reaction plate surge force
