...
%
% :param roots: roots
% :param idx_no_sat: idx_no_sat
% :param idx_zero: idx_zero
% :param a_quad: a_quad
% :param b_quad: b_quad
% :param c_quad: c_quad
% :returns: mult
    ...
    if any(both_ok,'all')       % two solutions
        ...
        if ~all(roots_equal(both_ok),'all')
            warning('MDOcean:PickWhichRoot:TwoValidSolutions', 'Two valid non-equal solutions, choosing 1st arbitrarily')
        end
        ...
        if ~(all( num_solns == 1,'all') ) % confirm that 1 soln per sea state meets criteria
            ...
            warning('MDOcean:PickWhichRoot:NoValidSolution', 'Some sea states have no valid quadratic solution, so their energy is zeroed.')        
        end
