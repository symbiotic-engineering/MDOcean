...
function [mag_U,phase_U,...
    ...
    % initialize outputs as NaN; NaN sea states (zero JPD) will remain NaN
    sz = size(w);
    [mag_U, phase_U, real_P, reactive_P, mag_X_u, phase_X_u, ...
     mag_X_f, phase_X_f, mag_X_s, phase_X_s, B_p, K_p, P_sat_ratio, ...
     F_drag_f, F_drag_s, phase_F_drag_f, phase_F_drag_s] = deal(NaN(sz));

    % per-sea-state convergence mask: NaN/invalid sea states are already "converged"
    ss_converged = ~isfinite(B_h_f);
    iters = 0;

    % loop until all valid sea states have converged individually
    while ~all(ss_converged(:))
        if drag_convergence_plot_on
            % record guesses
        ...
        % compute dynamics only for sea states that have not yet converged
        idx_active = ~ss_converged;
        sub = @(M) M(idx_active);

        [X_f_err_a, X_s_err_a, ...
         phase_X_f_err_a, phase_X_s_err_a, ...
         force_lim_err_a, amp_lim_err_a, ...
         mag_U_a, phase_U_a, ...
         real_P_a, reactive_P_a, ...
         mag_X_u_a, phase_X_u_a, ...
         mag_X_f_a, phase_X_f_a, ...
         mag_X_s_a, phase_X_s_a, ...
         B_p_a, K_p_a, P_sat_ratio_a, ...
         F_drag_f_a, F_drag_s_a, ...
         phase_F_drag_f_a, phase_F_drag_s_a] = dynamics_error_wrapper( ...
                                            sub(X_f_guess), sub(X_s_guess), ...
                                            sub(phase_X_f_guess), sub(phase_X_s_guess), ...
                                            sub(ctrl_mult_guess), sub(phase_ctrl_mult_guess), ...
                                            sub(H), sub(w), sub(k_wvn), ...
                                            sub(m_f), sub(m_s), sub(m_c), ...
                                            sub(B_h_f), sub(B_h_s), sub(B_c), ...
                                            sub(K_h_f), sub(K_h_s), ...
                                            sub(gamma_f_mag), sub(gamma_f_phase), ...
                                            sub(gamma_s_mag), sub(gamma_s_phase), F_max, ...
                                            drag_const_f, drag_const_s, ...
                                            sub(mag_v0_f), sub(mag_v0_s), ...
                                            X_max, control_type, multibody, merge_bodies, ...
                                            drag_fcn, D_f, D_f_in, D_d, ...
                                            T_f_slam, T_s_slam);

        % write active-subset results back into the full-size output arrays
        mag_U(idx_active)          = mag_U_a;
        phase_U(idx_active)        = phase_U_a;
        real_P(idx_active)         = real_P_a;
        reactive_P(idx_active)     = reactive_P_a;
        mag_X_u(idx_active)        = mag_X_u_a;
        phase_X_u(idx_active)      = phase_X_u_a;
        mag_X_f(idx_active)        = mag_X_f_a;
        phase_X_f(idx_active)      = phase_X_f_a;
        mag_X_s(idx_active)        = mag_X_s_a;
        phase_X_s(idx_active)      = phase_X_s_a;
        B_p(idx_active)            = B_p_a;
        K_p(idx_active)            = K_p_a;
        P_sat_ratio(idx_active)    = P_sat_ratio_a;
        F_drag_f(idx_active)       = F_drag_f_a;
        F_drag_s(idx_active)       = F_drag_s_a;
        phase_F_drag_f(idx_active) = phase_F_drag_f_a;
        phase_F_drag_s(idx_active) = phase_F_drag_s_a;

        % update guesses for active sea states only
        X_f_guess(idx_active)             = mag_X_f_a;
        X_s_guess(idx_active)             = mag_X_s_a;
        phase_X_f_guess(idx_active)       = phase_X_f_a;
        phase_X_s_guess(idx_active)       = phase_X_s_a;
        ctrl_mult_guess(idx_active)       = sub(ctrl_mult_guess) ./ (force_lim_err_a + 1);
        phase_ctrl_mult_guess(idx_active) = eps; % fixme: If reactive control, phase should be nonzero, based on eq4 of IFAC paper.

        % per-sea-state convergence: mark sea states whose errors are all below tolerance
        newly_converged = abs(X_f_err_a)       < X_tol       & ...
                          abs(X_s_err_a)       < X_tol       & ...
                          abs(phase_X_f_err_a) < phase_X_tol & ...
                          abs(phase_X_s_err_a) < phase_X_tol & ...
                          abs(force_lim_err_a) < F_lim_tol   & ...
                          abs(amp_lim_err_a)   < X_lim_tol;
        ss_converged(idx_active) = ss_converged(idx_active) | newly_converged;

        if all(isnan(mag_X_f_a))
            error('all nan')
        end
