function [mag_U,phase_U,...
    ...
    phase_F_drag_f,phase_F_drag_s,...
         ...
         assert(islogical(compute_qcqp_debug) && isscalar(compute_qcqp_debug), ...
                'get_response_drag:InvalidComputeQcqpDebugFlag', ...
    ...
    if max_drag_iters_solver > 0
        num_solver_outputs = 17 + double(compute_qcqp_debug);
        solver_outputs = cell(1, num_solver_outputs);
        [solver_outputs{:}] = solver(X_f_guess,X_s_guess,...
                                        phase_X_f_guess,phase_X_s_guess,...
                                        ctrl_mult_guess,phase_ctrl_mult_guess,...
        ...
        [mag_U,phase_U,...
         ...
         B_p,K_p,P_sat_ratio,...
         F_drag_f,F_drag_s,...
         phase_F_drag_f,phase_F_drag_s] = solver_outputs{1:17};
        if compute_qcqp_debug
            qcqp_debug = solver_outputs{18};
        end
    else
        if compute_qcqp_debug
...
function [mag_U,phase_U,...
     ...
     phase_F_drag_f,...
     phase_F_drag_s,...
     qcqp_debug] = solver(X_f_guess,X_s_guess,phase_X_f_guess,phase_X_s_guess,...
                                                ctrl_mult_guess,phase_ctrl_mult_guess,...
    ...
    drag_convergence_plot_on= drag_ctrl_plot_on;
    opt_ctrl_plot_debug_on = drag_ctrl_plot_on;
    ...
    num_dyn_outputs = 17 + double(compute_qcqp_debug);
    dyn_outputs = cell(1, num_dyn_outputs);
    [dyn_outputs{:}] = dynamics_from_guess(X_f_solved, phase_X_f_solved, mag_v0_f, drag_const_f, ...
    ...
    [mag_U,phase_U,...
     ...
     F_drag_f,F_drag_s,...
     phase_F_drag_f,...
     phase_F_drag_s] = dyn_outputs{1:17};
    if compute_qcqp_debug
        qcqp_debug = dyn_outputs{18};
    end
...
function [X_f_err,X_s_err,...
    ...
    [mag_U,phase_U,...
     ...
     force_lim_err,amp_lim_err] = dynamics_from_guess(X_f_guess, phase_X_f_guess, mag_v0_f, drag_const_f, ...
                                    ...
                                    drag_fcn, D_f, D_f_in, D_d, T_f_slam, T_s_slam, false); % closed loop dynamics; no polar plot or debug warnings during fsolve iterations
    % error
...
function [mag_U,phase_U,...
     ...
     phase_F_drag_f,phase_F_drag_s,...
        force_lim_err,...
        amp_lim_err,...
        ctrl_mult_best,phase_ctrl_mult_best,...
        qcqp_debug] = dynamics_from_guess(X_f_guess, phase_X_f_guess, mag_v0_f, drag_const_f, ...
                                         X_s_guess, phase_X_s_guess, mag_v0_s, drag_const_s, ...
                                         B_c,B_h_f,B_h_s,K_h_f,K_h_s,m_c,m_f,m_s,H,w,k_wvn,...
                                         gamma_f_mag,gamma_f_phase,gamma_s_mag,gamma_s_phase,...
                                         control_type,multibody,merge_bodies,F_max,P_max,X_max,...
                                         control_solve_type,ctrl_mult_guess,phase_ctrl_mult_guess,...
                                         drag_fcn,...
                                         D_f, D_f_in, D_d, T_f_slam, T_s_slam, opt_ctrl_plot_debug_on)
...
function ang = my_angle(complex_term) % also defined in constrained_opt_ctrl
