...
function [opt_mag_U,opt_phase_U,...
    ...
    phase_U = nan([size(w), numel(MAG_GUESS)]);
    real_P = nan([size(w), numel(MAG_GUESS)]);
    reactive_P = nan([size(w), numel(MAG_GUESS)]);
    mag_X_u = nan([size(w), numel(MAG_GUESS)]);
    phase_X_u = nan([size(w), numel(MAG_GUESS)]);
    mag_X_f = nan([size(w), numel(MAG_GUESS)]);
    phase_X_f = nan([size(w), numel(MAG_GUESS)]);
    mag_X_s = nan([size(w), numel(MAG_GUESS)]);
    phase_X_s = nan([size(w), numel(MAG_GUESS)]);
    B_p_sat = nan([size(w), numel(MAG_GUESS)]);
    K_p_sat = nan([size(w), numel(MAG_GUESS)]);
    constraint_err = nan([size(w), numel(MAG_GUESS)]);
    MAG_STAB_arr   = nan([size(w), numel(MAG_GUESS)]);
    PHASE_STAB_arr = nan([size(w), numel(MAG_GUESS)]);
    real_P_stab_arr = nan([size(w), numel(MAG_GUESS)]);

    for i = 1:numel(MAG_GUESS)
        [mag_U(:,:,i),phase_U(:,:,i),...
        real_P(:,:,i),reactive_P(:,:,i),...
        mag_X_u(:,:,i),phase_X_u(:,:,i),...
        mag_X_f(:,:,i),phase_X_f(:,:,i),...
        mag_X_s(:,:,i),phase_X_s(:,:,i),...
        B_p_sat(:,:,i),K_p_sat(:,:,i),...
        constraint_err(:,:,i),...
        ~,...
        MAG_STAB_arr(:,:,i),PHASE_STAB_arr(:,:,i),real_P_stab_arr(:,:,i)] = ...
            response_and_ctrl_err_from_ctrl_guess(MAG_GUESS(i),...
                                                    PHASE_GUESS(i),...
    ...
    if brute_force_plot_on
        make_ctrl_polar_plot(MAG_GUESS, PHASE_GUESS, real_P_for_plot, constraint_err, idx_opt, H, 2*pi./w, ...
                             MAG_STAB_arr, PHASE_STAB_arr, real_P_stab_arr);
    end
    ...
    opt_phase_X_u = phase_X_u(idx_opt);
    opt_mag_X_f = mag_X_f(idx_opt);
    opt_phase_X_f = phase_X_f(idx_opt);
    opt_mag_X_s = mag_X_s(idx_opt);
    opt_phase_X_s = phase_X_s(idx_opt);
    opt_B_p_sat = B_p_sat(idx_opt);
    opt_K_p_sat = K_p_sat(idx_opt);
end

function [mag_U,phase_U,real_P,reactive_P,...
        mag_X_u,phase_X_u,mag_X_f,phase_X_f,...
        mag_X_s,phase_X_s,B_p_sat,K_p_sat,...
        constr_viol_err,optimality_err,...
        MAG_STAB,PHASE_STAB,real_P_stab] = response_and_ctrl_err_from_ctrl_guess(...
                                                    ctrl_mult_guess,...
    ...
    % stabilized positions: evaluate with B/K stabilization active to find where
    % the controller actually lands after stabilization (may differ from the
    % unstabilized grid point when the system needs stabilization)
    if nargout > 14
        [~,~,real_P_stab,~,~,~,~,~,~,~,B_l_stab,K_l_stab] = ...
            control_evaluation_fcn(K_p_sat, B_p_sat, true, true);
        Z_p_stab_actual = B_l_stab - 1i * K_l_stab ./ w;
        denom_safe = abs(Z_p) > eps;
        mult_stab = ones(size(Z_p_stab_actual));
        mult_stab(denom_safe) = Z_p_stab_actual(denom_safe) ./ Z_p(denom_safe);
        MAG_STAB   = abs(mult_stab);
        PHASE_STAB = angle(mult_stab);
    end
end
...
function [constr_viol_err,optimality_err] = control_errors_from_sat_results(ctrl_mult_mag,...
                                                   ctrl_mult_phase, F_max, P_max, mag_U_unsat, mag_U,...
