...
function [mag_U,phase_U,...
    ...
    if num_solver_vars == 6
        ...
        ctrl_mult_solved = nan(size(w));
        phase_ctrl_mult_solved = nan(size(w));
    end
    ...
    [mag_U,phase_U,...
     ...
     phase_F_drag_f,...
     phase_F_drag_s,...
     ctrl_mult_best,phase_ctrl_mult_best] = dynamics_from_guess(X_f_solved, phase_X_f_solved, mag_v0_f, drag_const_f, ...
    ...
    if num_solver_vars ~= 6
        ctrl_mult_solved = ctrl_mult_best;
        phase_ctrl_mult_solved = phase_ctrl_mult_best;
    end
...
function [mag_U,phase_U,...
     ...
     phase_F_drag_f,phase_F_drag_s,...
         force_lim_err,...
         amp_lim_err,...
         ctrl_mult_best,phase_ctrl_mult_best] = dynamics_from_guess(X_f_guess, phase_X_f_guess, mag_v0_f, drag_const_f, ...
                                        X_s_guess, phase_X_s_guess, mag_v0_s, drag_const_s, ...
                                        B_c,B_h_f,B_h_s,K_h_f,K_h_s,m_c,m_f,m_s,H,w,k_wvn,...
                                        gamma_f_mag,gamma_f_phase,gamma_s_mag,gamma_s_phase,...
                                        control_type,multibody,merge_bodies,F_max,P_max,X_max,...
    ...
    elseif strcmpi(control_solve_type,'brute_force')
        ...
        error('Invalid control_solve_type: %s. Supported values are ''solver'' or ''brute_force''.', control_solve_type);
    end
