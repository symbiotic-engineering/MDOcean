...
function [B,G_real,G_imag] = drag_LUT_or_analytical_or_compute(R,TH,K,LUT_fcn)
    ...
    if any(idx_nan(:))
        rp_vals    = 1 ./ R(idx_nan);
        kappa_vals = K(idx_nan);
        warning('MDOcean:DragIntegral:OutsideLUT', ...
            ['Computing drag integral numerically: rp_min=%.6g, rp_max=%.6g, ' ...
             'kappa_min=%.6g, kappa_max=%.6g because they are outside the range of the ' ...
             'precomputed lookup table. If you see this message often and want to reduce ' ...
             'compute time, go to calkit.yaml, find the call to make_drag_integral_LUT(), ' ...
             'and adjust the rp_min, rp_max, and/or kappa_max inputs to expand the lookup table.'], ...
            min(rp_vals), max(rp_vals), min(kappa_vals), max(kappa_vals))
