...
function [mag_U,phase_U,...
    ...
    % anonymous function to take collapsed vector and turn into 4 sea state matrices
    sz = size(w);
    N_ss = prod(sz); % number of sea states
    unflatten = @(x) deal( reshape( x(1:N_ss)         , sz),...
                           reshape( x(N_ss+1:2*N_ss)  , sz),...
                           reshape( x(2*N_ss+1:3*N_ss), sz),...
                           reshape( x(3*N_ss+1:end)   , sz) ...
                         );
    % anonymous function to take 4+ sea state matrices and turn into collapsed vector of the first 4
    flatten = @(X1,X2,X3,X4,varargin) [X1(:),X2(:),X3(:),X4(:)];

    % prepare inputs for solver
    x0 = flatten(X_f_guess,X_s_guess,phase_X_f_guess,phase_X_s_guess);

    fun = @(x) flatten( dynamics_error_wrapper(unflatten(x),...
                                    w,m_f,m_s,m_c,B_h_f,B_h_s,B_c,K_h_f,K_h_s,...
                                    F_f_mag,F_f_phase,F_s_mag,F_s_phase,F_max,...
                                    drag_const_f,drag_const_s,mag_v0_f,mag_v0_s,...
                                    X_max,control_type,multibody)...
