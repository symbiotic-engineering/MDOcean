...
function [mag_U,phase_U,...
    ...
    [mag_U,phase_U,...
     ...
     mag_X_f,phase_X_f,...
     mag_X_s,phase_X_s,...
     B_p_sat,K_p_sat] = get_response_sat_iterate_control(B_c,B_f,B_s,K_f,K_s,...
...
function [mag_U,phase_U,...
     ...
     mag_X_s,phase_X_s,...
     B_p_sat,K_p_sat] = get_response_sat_iterate_control(B_c,B_f,B_s,K_f,K_s,...
                                                m_c,m_f,m_s,w,K_p_start,B_p_start,...
                                                F_f_mag,F_f_phase,...
                                                F_s_mag,F_s_phase,...
                                                F_max,X_max,multibody)

    if length(K_p_start)>1
        B_p_multiplier_damping = sqrt(1+(K_p_start(5,4)/(w(5,4)*B_p_start(5,4)))^2)
    else
        B_p_multiplier_damping = 1.17;
    end
    B_p_multiplier = [.1:.1:3 B_p_multiplier_damping];
    num_B = length(B_p_multiplier);

    if length(K_p_start) == 1 % damping control or storm - don't sweep K (hack)
        num_K = 1;
        K_p_multiplier = 1;
    else
        K_p_multiplier = [1e-8/K_p_start(5,4) .1:.1:2];
        num_K = length(K_p_multiplier);
    end
    
    num_H = size(w,1); % number of wave heights
    num_T = size(w,2); % number of wave periods

    P = zeros(num_B,num_K,num_H,num_T); 

    % try each B/K multiplier for all sea states
    for idx_B = 1:num_B
        B_p = B_p_multiplier(idx_B) * B_p_start;

        for idx_K = 1:num_K
    
            K_p = K_p_multiplier(idx_K) * K_p_start;

            [~,~,P(idx_B,idx_K,:,:)] = get_response_saturated(B_c,B_f,B_s,K_f,K_s,...
                                                            m_c,m_f,m_s,w,K_p,B_p,...
                                                            F_f_mag,F_f_phase,...
                                                            F_s_mag,F_s_phase,...
                                                            F_max,X_max,multibody);
    ...
    % evaluate what B/K is best for this sea state
    [~,idxs_best_linear] = max(P,[],[1,2],'linear'); % take the max over B and K indexes
    [idxs_best_B,idxs_best_K,~,~] = ind2sub(size(P),idxs_best_linear);
    
    idxs_best_B = squeeze(idxs_best_B);
    idxs_best_K = squeeze(idxs_best_K);

    B_p_mult_best = B_p_multiplier(idxs_best_B);
    K_p_mult_best = K_p_multiplier(idxs_best_K);

    B_p_best = B_p_mult_best .* B_p_start;
    K_p_best = K_p_mult_best .* K_p_start;

    if ~all(P<0,'all') && (any(K_p_mult_best ~= 1,'all') || any(B_p_mult_best ~= 1,'all'))
        disp("optimal control isn't actually optimal")
    end
