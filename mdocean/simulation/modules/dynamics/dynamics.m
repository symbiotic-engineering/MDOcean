...
function [mag_U,phase_U,...
    ...
    if max_drag_iters_fixed_point > 0
        [mag_U,phase_U,...
         ...
         idx_converged] = fixed_point_iteration(X_f_guess,X_s_guess,...
                                                    phase_X_f_guess,phase_X_s_guess,...
                                                    ctrl_mult_guess,phase_ctrl_mult_guess,...
                                                    dynam_inputs{:}, ...
                                                    max_drag_iters_fixed_point);

        %idx_reasonable = (ctrl_mult >= 0 & ctrl_mult < 1e4) | isnan(ctrl_mult);
        idx_use = idx_converged; % | idx_reasonable;
...
function [mag_U,phase_U,...
     ...
     idx_converged] = fixed_point_iteration(X_f_guess,X_s_guess,phase_X_f_guess,phase_X_s_guess,...
                                                ctrl_mult_guess,phase_ctrl_mult_guess,...
                                                w,m_f,m_s,m_c,B_h_f,B_h_s,B_c,K_h_f,K_h_s,...
                                                F_f_mag,F_f_phase,F_s_mag,F_s_phase,F_max,...
                                                drag_const_f,drag_const_s,mag_v0_f,mag_v0_s,...
    ...
    while ~converged
        ...
        [X_f_err,X_s_err,...
         ...
         B_p,K_p,P_sat_ratio] = dynamics_error_wrapper(X_f_guess,X_s_guess,...
                                                ...
                                                drag_const_f,drag_const_s,mag_v0_f,mag_v0_s,...
                                                X_max,control_type,multibody);
        % matrix error (all sea states)
        X_err_mat       = max( abs(X_f_err), ...
                               abs(X_s_err) );
        phase_X_err_mat = max( abs(phase_X_f_err), ...
                               abs(phase_X_s_err) );
        F_lim_err_mat   =      abs(force_lim_err);
        X_lim_err_mat   =      abs(amp_lim_err);

        % scalar error (worst sea state)
        X_err       = max( X_err_mat,       [], 'all');
        phase_X_err = max( phase_X_err_mat, [], 'all');
        F_lim_err   = max( F_lim_err_mat,   [], 'all');
        X_lim_err   = max( X_lim_err_mat,   [], 'all');
        ...
        idx_converged = X_err_mat < X_tol & ...
                        phase_X_err_mat < phase_X_tol & ...
                        F_lim_err_mat < F_lim_tol & ...
                        X_lim_err_mat < X_lim_tol;
    end
