...
function [F_heave_storm, F_surge_storm, ...
    ...
    ol_unstable] = dynamics(in,m_float,m_spar)
% Function dynamics
%
% :param in: in
...
% :returns: phase_X_u
    ...
    ol_unstable] = get_power_force(in,T,Hs,m_float,m_spar,...
...
function [P_matrix, X_constraints, B_p, K_p, ...
        ...
        F_drag_f, F_drag_s, phase_F_drag_f, phase_F_drag_s,...
        ol_unstable] = get_power_force(in,T,Hs, m_float, m_spar, ...
                                                            F_max, idx_ignore_constraint, T_f_slam, power_scale_coeffs,...
    ...
    [m_f,B_h_f,K_h_f,gamma_f_mag,gamma_f_phase,...
     ...
     gamma_f_over_rho_g, gamma_s_over_rho_g] = get_dynamic_coeffs(Hs,T, ...
                                            ...
                                            in.rho_w, in.g, in.use_MEEM, ...
                                            in.harmonics, in.hydro, m_k_h_precomputed);

    X_u_max = 1e6;%min(Hs / (2*sqrt(2)), in.T_f);

    % open loop stability check (using hydrodynamic coefficients, without drag)
    % positive definiteness of mass, damping, and stiffness matrices
    if merge_bodies
        ol_stable = (m_f + 2*m_c + m_s > 0) & (B_h_f + 2*B_c + B_h_s > 0) & (K_h_f + K_h_s > 0);
    elseif in.use_multibody
        % positive definiteness of 2x2 matrices (inline check_posdef_three_2x2s logic)
        ol_stable = m_f > 0 & (m_f.*m_s - m_c.^2 >= 0) & ...
                    B_h_f > 0 & (B_h_f.*B_h_s - B_c.^2 >= 0) & ...
                    K_h_f > 0 & (K_h_f.*K_h_s >= 0);
    else
        ol_stable = (m_f > 0) & (B_h_f > 0) & (K_h_f > 0);
    end
