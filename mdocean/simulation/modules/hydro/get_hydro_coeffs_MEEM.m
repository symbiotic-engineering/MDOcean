...
function [A_f_over_rho, A_s_over_rho, A_c_over_rho, ...
        ...
        gamma_phase_f, gamma_phase_s] = get_hydro_coeffs_MEEM(a2, m0, d2, a1, d1, a3, h, g, w, harmonics, ...
                                                              spar_excitation_coeffs, m_k_h_precomputed)
...
if nargin < 12
    ...
    if size(m_k_h_precomputed,1) == numel(m0_meem)
        m_k_h_for_meem = m_k_h_precomputed;
    elseif size(m_k_h_precomputed,1) == size(m0,2)
        active_period_mask = any(~isnan(m0),1);
        m_k_h_for_meem = m_k_h_precomputed(active_period_mask,:);
    else
        error('MDOcean:GetHydroCoeffsMEEM:InvalidPrecomputedMKHSize', ...
            ['m_k_h_precomputed row count does not match active periods in m0. ' ...
             'Expected %d or %d rows, got %d.'], ...
            numel(m0_meem), size(m0,2), size(m_k_h_precomputed,1));
    end
    [mu_nondim, lambda_nondim, gamma_phase_f] = run_MEEM(heaving_IC, heaving_OC, auto_BCs, ...
                                                   N_num, M_num, K_num, ...
                                                   a1/h, a2/h, d1/h, ...
