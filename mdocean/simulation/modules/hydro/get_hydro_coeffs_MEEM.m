...
if nargin < 12
    ...
    if size(m_k_h_for_meem,1) ~= numel(m0_meem)
        active_period_cols = any(~isnan(m0),1);
        if size(m_k_h_for_meem,1) == size(m0,2)
            m_k_h_for_meem = m_k_h_for_meem(active_period_cols,:);
        else
            error('MDOcean:GetHydroCoeffsMEEM:InvalidPrecomputedMKHSize', ...
                ...
                numel(m0_meem), size(m0,2), size(m_k_h_for_meem,1));
        end
    end
    [mu_nondim, lambda_nondim, gamma_phase_f] = run_MEEM(heaving_IC, heaving_OC, auto_BCs, ...
                                                   N_num, M_num, K_num, ...
