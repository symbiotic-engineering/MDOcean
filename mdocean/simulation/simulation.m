%[DOCS AUTOGENERATED]
function [J, P_matrix_elec, g_vec, val] = simulation(X, p, return_val)	
% Function simulation
%
...
% :returns: g_vec
% :param return_val: Logical flag to force expensive validation outputs
% :returns: Simulation result values struct

if nargin < 3
    return_val = (nargout > 3);
end
assert(islogical(return_val) && isscalar(return_val), ...
       'simulation:InvalidReturnValFlag', ...
       'return_val must be a logical scalar.');
val = [];
...
g_vec(3) = V_s_pct;                         % prevent spar too heavy
g_vec(4) = 1 - V_s_pct;                     % prevent spar too light
g_vec(5) = GM;                              % pitch stability of float-spar system
g_vec(6) = FOS_float(1) / p.FOS_min - 1;    % float survives max force
g_vec(7) = FOS_float(2) / p.FOS_min - 1;    % float survives fatigue
g_vec(8) = FOS_spar(1) / p.FOS_min - 1;     % spar survives max force
g_vec(9) = FOS_spar(2) / p.FOS_min - 1;     % spar survives fatigue
g_vec(10) = FOS_damping_plate(1) * in.FOS_mult_d / p.FOS_min - 1; % damping plate survives max force
g_vec(11) = FOS_damping_plate(2) * in.FOS_mult_d / p.FOS_min - 1; % damping plate survives fatigue
g_vec(12) = FOS_spar_local(1) / p.FOS_min - 1;    % spar survives max force in local buckling
g_vec(13) = FOS_spar_local(2) / p.FOS_min - 1;    % spar survives fatigue in local buckling
if ~isfield(p,'avg_power_min') || p.avg_power_min == 0
    g_vec(14) = P_avg_elec/1e6;                     % positive power
else
...
no_imag = all(isreal([g_vec LCOE P_var]));
...
if return_val % if returning extra struct output for validation
    [~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~,~,...
