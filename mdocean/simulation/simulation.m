...
[V_d, m_m, m_f_tot, ...
    ...
    I, T, V_f_pct, V_s_pct, GM] = geometry(in.D_s, in.D_f, in.T_f_1, in.T_f_2, in.h_f, in.h_s, ...
                                            in.t_ft, in.t_fr, in.t_fc, in.t_fb, in.t_sr, ...
                                            in.t_dt, D_d, in.D_dt, in.theta_dt, in.T_s, in.h_d, ...
...
if nargout > 4 % if returning extra struct output for validation
    [~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, mass] = geometry(in.D_s, in.D_f, in.T_f_1, in.T_f_2,...
    ...
    [~, ~, ~, ~, ~, ~, ~, P_unsat,B_p,X] = dynamics(in, m_f_tot, V_d, T);
    val.mass_f  = mass(1);
    val.mass_vc = mass(2);
    val.mass_rp = mass(3);
