...
in.t_ft  = X(8);     % float top thickness (m)
in.t_fr  = X(9);     % float radial wall thickness (m)
in.t_fc  = X(10);    % float circumferential gusset thickness (m)
in.t_fb  = X(11);    % float bottom thickness (m)
in.t_sr  = X(12);    % vertical column thickness (m)
in.t_dt  = X(13);    % damping plate support tube radial wall thickness (m)      
in.P_max = X(14);    % maximum power (W)
in.M     = X(15);    % material (-)
