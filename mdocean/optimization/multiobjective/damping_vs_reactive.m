%[DOCS AUTOGENERATED]
function [r1,r2] = damping_vs_reactive(p,b)
% Function damping_vs_reactive
%
% :param p: Parameter struct
% :param b: Design variable bounds struct
% :returns: r1
% :returns: r2

    p.control_type = 'damping';
    [x, fval, residuals, tol, p, paretoPctTimeSeeds, paretoPctTimePatternSearch, paretoTotalRuntime, paretoFcnEvals] = pareto_search(p, b, b.filename_uuid);
    
    p2 = p;
    p2.control_type = 'reactive';
    [x2,fval2,residuals2,tol2,p2] = pareto_search(p2, b, b.filename_uuid);

    % Store results
    r1.x = x;
    r1.fval = fval;
    r1.residuals = residuals;
    r1.tol = tol;
    r1.p = p;
    r1.paretoPctTimeSeeds = paretoPctTimeSeeds;
    r1.paretoPctTimePatternSearch = paretoPctTimePatternSearch;
    r1.paretoTotalRuntime = paretoTotalRuntime;
    r1.paretoFcnEvals = paretoFcnEvals;

    r2.x = x2;
    r2.fval = fval2;
    r2.residuals = residuals2;
    r2.tol = tol2;
    r2.p = p2;

end