%[DOCS AUTOGENERATED]
function [X,vals,all_flags] = compare_run(p,b)
...
[Xs_opt,~,flags,~,~,~,~,vals_opt] = gradient_optim(x0_input,p,b);
X_minLCOE = Xs_opt(:,1);
X_minCapex = Xs_opt(:,2);
val_minLCOE = vals_opt(1);
val_minCapex = vals_opt(2);
X_maxPower = Xs_opt(:,3);
val_maxPower = vals_opt(3);

%[X_maxPower,val_maxPower] = max_avg_power(p,b,X_minLCOE);

p_bal = p;
p_bal.avg_power_min = b.power_balanced;
[X_balanced,~,flag_balanced,~,~,~,~,val_balanced] = gradient_optim(x0_input,p_bal,b,2);
