%[DOCS AUTOGENERATED]
function [fig, fig_gamma] = make_ctrl_polar_plot(MAG_GUESS, PHASE_GUESS, real_P, constraint_err, idx_opt, H_ss, T_ss, MAG_STAB, PHASE_STAB, real_P_stab)
...
% :param MAG_STAB:       (optional) stabilized ctrl mult magnitudes (n_Hs x n_T x n_ctrl)
% :param PHASE_STAB:     (optional) stabilized ctrl mult phases in rad (n_Hs x n_T x n_ctrl)
% :param real_P_stab:    (optional) power at stabilized controllers (n_Hs x n_T x n_ctrl)
...
% :returns: fig_gamma Polar gamma-space figure handle
    ...
    have_stab = nargin >= 10 && ~isempty(MAG_STAB) && ~isempty(PHASE_STAB) && ~isempty(real_P_stab);
    ...
    % (i.e. MAG_STAB ~= MAG_GUESS or PHASE_STAB ~= PHASE_GUESS for the chosen sea state).
    STAB_TOL = 1e-6;
    if have_stab
        n_stab = numel(MAG_GUESS);
        MAG_STAB_flat  = reshape(MAG_STAB,  n_ss, n_stab);
        PHASE_STAB_flat = reshape(PHASE_STAB, n_ss, n_stab);
        P_stab_flat     = reshape(real_P_stab, n_ss, n_stab);
        ...
        P_stab_ss     = reshape(P_stab_flat(idx_chosen_ss, :),     sz_phase, sz_mag);

        % keep only grid points where stabilisation shifted the controller
        is_shifted = (abs(MAG_STAB_ss - MAG_GUESS)     > STAB_TOL | ...
                      abs(PHASE_STAB_ss - PHASE_GUESS)  > STAB_TOL) & ...
                     isfinite(MAG_STAB_ss) & isfinite(PHASE_STAB_ss);

        if any(is_shifted(:))
            r_stab  = log10(MAG_STAB_ss(is_shifted)) - log10(min_mag);
            phi_stab = PHASE_STAB_ss(is_shifted);
            x_stab  = r_stab .* cos(phi_stab);
            y_stab  = r_stab .* sin(phi_stab);
            p_stab_vals = P_stab_ss(is_shifted);
    ...
    if have_stab && any(is_shifted(:))
        alpha_stab_pts = MAG_STAB_ss(is_shifted) .* exp(1i .* PHASE_STAB_ss(is_shifted));
