classdef (Abstract) GenericAnalysis < handle
    ...
    methods
        ...
        function cmd = wrap_stage_command(~, inner)
            %WRAP_STAGE_COMMAND Wrap MATLAB code in ./mymatlab, which starts
            %   MATLAB (auto-licensed on CI) and runs the code with the
            %   locked dependencies from mip.yaml loaded.
            cmd = ['./mymatlab "' inner '"'];
        end
